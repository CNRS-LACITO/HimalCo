...
chomp($donnees);
	...
	if ($donnees =~ m/^\\lx/) {
			...
			if ($counter > 0) {
				print FICHIER2 "\\end{itemize}\n\n"; #end of the preceding examples
				$counter = 0; #reinitialize the counter
			}
			
			$donnees =~ s/\\lx//;
			$donnees =~ s/ //;
			print FICHIER2 "\\section*{\\nep{".transcr(infinitif($donnees))."}}\n";
			print FICHIER2 "\\ipa{\\textbf{".(infinitif($donnees))."}}\n";
	...
	if ($donnees =~ m/^\\xv/) {
			...
			$donnees =~ s/ //;
			if ($counter == 0) {
				print FICHIER2 "\\begin{itemize}\n"; #begin of the  example section
			}
			print FICHIER2 "\\item \\ipa{".$donnees."}\n";
			print FICHIER2 "  \\nep{".transcr($donnees)."}\n";
			print FICHIER2 "\n";
			$counter++;
	}
