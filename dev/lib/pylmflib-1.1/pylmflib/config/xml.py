...
from pylmflib.core.lexical_resource import LexicalResource
from pylmflib.core.lexicon import Lexicon
from pylmflib.utils.xml_format import parse_xml
from pylmflib.utils.error_handling import InputError
from pylmflib.config.mdf import mdf_lmf, lmf_mdf, mdf_order, ps_partOfSpeech, pdl_paradigmLabel, pd_grammaticalNumber, pd_person, pd_anymacy, pd_clusivity
from pylmflib.config.tex import partOfSpeech_tex, paradigmLabel_tex
from pylmflib.common.range import partOfSpeech_range, paradigmLabel_range

# If an LMF module needs to access languages or fonts, copy following lines:
# import pylmflib.config
# print pylmflib.config.xml.vernacular, pylmflib.config.xml.English, pylmflib.config.xml.national, pylmflib.config.xml.regional, pylmflib.config.xml.French
# print pylmflib.config.xml.font
    ...
    for format in configuration:
        if format.tag == "Language":
            ...
            for lang in format:
                ...
                exec("pylmflib.config.xml." + lang.attrib["att"] + " = '" + lang.attrib["val"] + "'")
        elif format.tag == "Font":
            pylmflib.config.xml.font = dict()
            ...
            for font in format:
                ...
                pylmflib.config.xml.font.update({font.attrib['att']: l})
        elif format.tag == "LMF":
                ...
                if object.tag == "GlobalInformation":
                    ...
                    for feat in object:
                        if feat.attrib["att"] == "language":
                            ...
                            pylmflib.config.xml.audio_path = os.path.abspath(os.path.abspath('.') + "/" + feat.attrib["val"]) + "/"
