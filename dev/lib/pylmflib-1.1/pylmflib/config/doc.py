...
from pylmflib.config.mdf import pd_grammaticalNumber, pd_person, pd_anymacy, pd_clusivity
from pylmflib.utils.error_handling import OutputError
from pylmflib.utils.io import ENCODING, EOL
        ...
        if not reverse:
            ...
            if len(lexical_entry.find_notes(type="grammar")) != 0:
                ...
                for note in lexical_entry.find_notes(type="grammar", language=pylmflib.config.xml.vernacular):
                    p.add_run(" ")
                    p.add_run(note)
                for note in lexical_entry.find_notes(type="grammar", language=pylmflib.config.xml.national):
                    p.add_run(" ")
                    p.add_run(note)
                # Italic
