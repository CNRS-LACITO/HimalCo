...
from pylmflib.config.tex import lmf_to_tex, partOfSpeech_tex
from pylmflib.config.mdf import mdf_semanticRelation, pd_grammaticalNumber, pd_person, pd_anymacy, pd_clusivity
from pylmflib.utils.io import open_read, open_write, EOL, ENCODING
from pylmflib.utils.error_handling import OutputError, Warning
...
def tex_write(object, filename, preamble=None, introduction=None, lmf2tex=lmf_to_tex, font=None, items=lambda lexical_entry: lexical_entry.get_lexeme(), sort_order=None, paradigms=[], tables=[], title=None, tex_language=None, tex_other_languages=[]):
    ...
    if tex_other_languages is not None:  # Ajouté par Benjamin pour permettre de proprement gérer plusieurs langues dans le fichier XeLaTeX.
        tex_file.write("\setotherlanguages{" + ", ".join(tex_other_languages) + "}" + EOL)
...
def handle_quotes(text):
    ...
    text = ""
    while result:
        text += result.group(1) + u"“" + result.group(2) + u"”"
    ...
    for form_representation in lexical_entry.get_form_representations():
        if form_representation.get_audio() is not None:
            ...
            if os.name == 'posix':
                ...
                file_path.append(pylmflib.config.xml.audio_path + "mp3/" + file_name)
                file_path.append(pylmflib.config.xml.audio_path + "wav/" + file_name)
            else:
                # Windows-style paths
...
def format_usage_notes(sense, font):
    ...
    """
    result = ""
    for usage in sense.find_usage_notes(language=pylmflib.config.xml.vernacular):
        result += "\\textit{VerUsage:} " + font[VERNACULAR](usage) + " "
    for usage in sense.find_usage_notes(language=pylmflib.config.xml.English):
    ...
    for usage in sense.find_usage_notes(language=pylmflib.config.xml.regional):
...
def format_encyclopedic_informations(sense, font):
    ...
    """
    result = ""
    for information in sense.find_encyclopedic_informations(language=pylmflib.config.xml.vernacular):
        result += font[VERNACULAR](information) + " "
    for information in sense.find_encyclopedic_informations(language=pylmflib.config.xml.English):
        result += information + " "
    for information in sense.find_encyclopedic_informations(language=pylmflib.config.xml.national):
        result += font[NATIONAL](handle_font(information)) + " "
    for information in sense.find_encyclopedic_informations(language=pylmflib.config.xml.regional):
...
def format_restrictions(sense, font):
    ...
    """
    result = ""
    for restriction in sense.find_restrictions(language=pylmflib.config.xml.vernacular):
        result += "\\textit{VerRestrict:} " + font[VERNACULAR](restriction) + " "
    for restriction in sense.find_restrictions(language=pylmflib.config.xml.English):
        result += "\\textit{Restrict:} " + restriction + " "
    for restriction in sense.find_restrictions(language=pylmflib.config.xml.national):
        result += "\\textit{" + font[NATIONAL](restriction) + "} "
    for restriction in sense.find_restrictions(language=pylmflib.config.xml.regional):
...
def format_variant_forms(lexical_entry, font):
    ...
    for form_representation in lexical_entry.get_form_representations():
        ...
        if form_representation.get_comment(pylmflib.config.xml.English) is not None:
            result +=  "(" + form_representation.get_comment(pylmflib.config.xml.English) + ") "
        if form_representation.get_comment(pylmflib.config.xml.national) is not None:
            result +=  "(" + font[NATIONAL](form_representation.get_comment(pylmflib.config.xml.national)) + ") "
        if form_representation.get_comment(pylmflib.config.xml.regional) is not None:
            result +=  "(" + font[REGIONAL](form_representation.get_comment(pylmflib.config.xml.regional)) + ") "
    return result
