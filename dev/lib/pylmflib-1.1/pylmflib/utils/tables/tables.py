...
parser.add_option("-c", "--output-cmn", dest="output_cmn", action="store", help="output LaTeX file in Chinese")
    ...
    if result:
        if result.group(1) == "lx" and result.group(2) == "*":
            ...
            if lx == "*":
                ...
                out_cmn.write("\\textcolor{brown}{\zh{" + gn + "}}")
                out_eng.write(" \\\\" + EOL)
                out_fra.write(" \\\\" + EOL)
                out_cmn.write(" \\\\" + EOL)
...
out_cmn.write("\\end{longtable}")
out_eng.write("\\end{center}")
out_fra.write("\\end{center}")
out_cmn.write("\\end{center}")
in_file.close()
out_eng.close()
out_fra.close()
out_cmn.close()
