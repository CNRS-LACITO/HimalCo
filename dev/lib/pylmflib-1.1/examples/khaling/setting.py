...
from pylmflib.config.mdf import mdf_lmf, mdf_semanticRelation, pd_grammaticalNumber, pd_person, pd_anymacy, pd_clusivity
...
def format_examples(sense, font):
    ...
    for context in sense.get_contexts():
        ...
        for example in context.find_written_forms(pylmflib.config.xml.vernacular, script_name="ipa"):
            result += "\\sn " + font[VERNACULAR](example) + EOL
        for example in context.find_written_forms(pylmflib.config.xml.vernacular, script_name="devanagari"): # xv_dev
            result += "\\trans " + font[NATIONAL](example) + EOL
        for example in context.find_written_forms(pylmflib.config.xml.English):
            result += "\\trans " + font[ENGLISH](example) + EOL
        for example in context.find_written_forms(pylmflib.config.xml.national):
            result += "\\trans " + font[NATIONAL](tex.handle_font(example)) + EOL
        for example in context.find_written_forms(pylmflib.config.xml.regional):
...
def format_paradigms(lexical_entry, font):
    ...
    for paradigm in lexical_entry.get_paradigms():
        if paradigm.get_paradigmLabel() is not None and paradigm.get_paradigm(language=pylmflib.config.xml.vernacular) is not None:
            if paradigm.get_paradigmLabel() != current_label:
                ...
                result += "; "
            result += font[VERNACULAR](paradigm.get_paradigm(language=pylmflib.config.xml.vernacular)) + " "
    return result
...
def lmf2tex(lexical_entry, font):
    ...
    # lexeme and id
    tex_entry += format_lexeme(lexical_entry, pylmflib.config.xml.font)
    # TODO: phonetic variants ? or variant form ?
    # sound
    tex_entry += tex.format_audio(lexical_entry, pylmflib.config.xml.font)
    # part of speech
    tex_entry += tex.format_part_of_speech(lexical_entry, pylmflib.config.xml.font)
    # grammatical notes
    tex_entry += format_notes(lexical_entry, pylmflib.config.xml.font)
    ...
    for sense in senses:
        ...
        tex_entry += tex.format_definitions(sense, pylmflib.config.xml.font, languages=[pylmflib.config.xml.vernacular, pylmflib.config.xml.English, pylmflib.config.xml.national])
        # example
        tex_entry += format_examples_compact(sense, pylmflib.config.xml.font)
        # usage note
        tex_entry += tex.format_usage_notes(sense, pylmflib.config.xml.font)
        # encyclopedic information
        tex_entry += tex.format_encyclopedic_informations(sense, pylmflib.config.xml.font)
        # restriction
        tex_entry += tex.format_restrictions(sense, pylmflib.config.xml.font)
    # synonym, antonym, morphology, related form
    tex_entry += tex.format_related_forms(lexical_entry, pylmflib.config.xml.font)
    # TODO: variant form?
    tex_entry += tex.format_variant_forms(lexical_entry, pylmflib.config.xml.font)
    # borrowed word
    tex_entry += tex.format_borrowed_word(lexical_entry, pylmflib.config.xml.font)
    # etymology
    tex_entry += tex.format_etymology(lexical_entry, pylmflib.config.xml.font)
    # paradigms
    tex_entry += format_paradigms(lexical_entry, pylmflib.config.xml.font)
    # semantic domain
    tex_entry += tex.format_semantic_domains(lexical_entry, pylmflib.config.xml.font)
    # TODO? bibliography
    tex_entry += tex.format_bibliography(lexical_entry, pylmflib.config.xml.font)
    # source
    tex_entry += tex.format_source(lexical_entry, pylmflib.config.xml.font)
    # status
    tex_entry += tex.format_status(lexical_entry, pylmflib.config.xml.font)
    # date
    tex_entry += tex.format_date(lexical_entry, pylmflib.config.xml.font)
    # Handle reserved characters and fonts
    tex_entry = handle_reserved(tex_entry)
    tex_entry = tex.handle_quotes(tex_entry)
    tex_entry = tex.handle_fv(tex_entry, pylmflib.config.xml.font)
    tex_entry = tex.handle_fn(tex_entry, pylmflib.config.xml.font)
    # Special formatting
