...
mdf_lmf.update({
    ...
    "xv"    : lambda xv, lexical_entry: lexical_entry.create_and_add_example(remove_char(xv), language=pylmflib.config.xml.vernacular),
...
def format_examples(sense, font):
    ...
    for context in sense.get_contexts():
        result += u"\u00B6 "
        for example in context.find_written_forms(pylmflib.config.xml.vernacular):
...
def format_paradigms(sense, font):
    from pylmflib.config.tex import paradigmLabel_tex
    result = ""
    current_label = None
...
def lmf2tex(lexical_entry, font):
    ...
    for sense in senses:
        ...
        tex_entry += format_definitions(sense, font, languages=[pylmflib.config.xml.vernacular, pylmflib.config.xml.French, pylmflib.config.xml.national])
        # example
    ...
    tex_entry += tex.format_related_forms(lexical_entry, font, language=pylmflib.config.xml.French)
    # borrowed word
