...
from pylmflib.config.mdf import ps_partOfSpeech, mdf_lmf, set_bw
from pylmflib.common.range import partOfSpeech_range
from pylmflib.config.tex import partOfSpeech_tex
from pylmflib.utils.io import EOL, ENCODING
from pylmflib.common.defs import VERNACULAR, NATIONAL, ENGLISH, REGIONAL
...
def get_gf(lexical_entry):
    for sense in lexical_entry.get_senses():
        for gloss in sense.find_glosses(language=pylmflib.config.xml.French):
...
def set_ce(ce, lexical_entry):
    ...
    if related_form is not None:
        related_form.create_and_add_form_representation(written_form=ce, language=pylmflib.config.xml.French)
...
mdf_lmf.update({
    ...
    # ge : French gloss
    "ge"    : lambda ge, lexical_entry: lexical_entry.set_gloss(force_caps(ge.replace('_', ' ').replace("GO(s)", "GOs").replace("GO(n)", "GOn").replace("WEM", "WE")), language=pylmflib.config.xml.French),
    # xn : French example
    "xn"    : lambda xn, lexical_entry: lexical_entry.add_example(force_caps(xn), language=pylmflib.config.xml.French),
    # xe : English example
    "xe"    : lambda xe, lexical_entry: lexical_entry.add_example(force_caps(xe), language=pylmflib.config.xml.English),
    # sge : French gloss of the subentry
    "sge"   : lambda sge, lexical_entry: lexical_entry.set_gloss(force_caps(sge), language=pylmflib.config.xml.French),
    # de : French definition
    "de"    : lambda de, lexical_entry: lexical_entry.set_definition(force_caps(de), language=pylmflib.config.xml.French),
    # gr : note grammaticale => [Note grammaticale : ] à la suite de [Note : ]
    "gr"    : lambda gr, lexical_entry: lexical_entry.set_note(gr, type="grammar", language=pylmflib.config.xml.regional),
    # gt: traduction de gr en français => [Note grammaticale : 'gr' (en gras) 'gt' (non gras)]
    "gt"    : lambda gt, lexical_entry: lexical_entry.set_note(force_caps(gt), type="grammar", language=pylmflib.config.xml.French),
    ...
    "ng"    : lambda ng, lexical_entry: lexical_entry.set_note(ng, type="grammar", language=pylmflib.config.xml.vernacular),
    ...
    "xv"    : lambda xv, lexical_entry: lexical_entry.create_and_add_example(xv.replace("GO(s)", "GOs").replace("GO(n)", "GOn").replace("WEM", "WE"), language=pylmflib.config.xml.vernacular),
    # cf : confer => OK
...
def lmf2doc(lexicon, document, items=lambda lexical_entry: lexical_entry.get_lexeme(), sort_order=None, paradigms=False, reverse=False):
    ...
    for lexical_entry in lexicon.get_lexical_entries():
        ...
        if not reverse:
            ...
            dialect = ""
            for sense in lexical_entry.get_senses():
                for usage_note in sense.find_usage_notes(language=pylmflib.config.xml.vernacular):
                    dialect += " [" + usage_note + "]"
            p = document.add_paragraph()
            ...
            if len(lexical_entry.find_notes(type="grammar")) != 0:
                ...
                for note in lexical_entry.find_notes(type="grammar", language=pylmflib.config.xml.regional):
                    p.add_run(" ")
                    p.add_run(note).bold = True
                try:
                    for note in lexical_entry.find_notes(type="grammar", language=pylmflib.config.xml.French):
                        p.add_run(" ")
                        p.add_run(note)
                except AttributeError:
                    for note in lexical_entry.find_notes(type="grammar", language=pylmflib.config.xml.English):
                        p.add_run(" ")
                        p.add_run(note)
                for note in lexical_entry.find_notes(type="grammar", language=pylmflib.config.xml.vernacular):
                    p.add_run(" ")
                    p.add_run(note)
                p.add_run("].")
            for sense in lexical_entry.get_senses():
                ...
                for gloss in sense.find_glosses(language=pylmflib.config.xml.vernacular):
                    glosses += " " + gloss + "."
                if glosses == "":
                ...
                try:
                    ...
                    for gloss in sense.find_glosses(language=pylmflib.config.xml.English):
                        glosses += " " + gloss + "."
                glosses = glosses.rstrip(".")
                ...
                for context in sense.get_contexts():
                    p = document.add_paragraph()
                    examples = ""
                    vernacular_forms = context.find_written_forms(language=pylmflib.config.xml.vernacular)
                    for example in vernacular_forms:
                        ...
                        for element in example.split('[')[1:]:
                            p.add_run('[' + element)
                    try:
                        fra_forms = context.find_written_forms(language=pylmflib.config.xml.French)
                ...
                if len(lexical_entry.get_related_forms("simple link")) != 0:
                    ...
                    for related_form in lexical_entry.get_related_forms("simple link"):
                        ...
                        for element in related_form.get_lexeme().split('[')[1:]:
                            p.add_run('[' + element)
                        try:
                            for written_form in related_form.find_written_forms(language=pylmflib.config.xml.French):
                                p.add_run(" " + written_form)
                        except AttributeError:
                            for written_form in related_form.find_written_forms(language=pylmflib.config.xml.English):
                                p.add_run(" " + written_form)
                    p.add_run(".")
                # Notes
...
def lmf2tex(lexical_entry, font):
    ...
    for sense in senses:
        ...
        tex_entry += tex.format_definitions(sense, font, languages=[pylmflib.config.xml.vernacular, pylmflib.config.xml.French, pylmflib.config.xml.national])
        # example
