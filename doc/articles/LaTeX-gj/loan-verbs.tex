...
\newcommand{\ipa}[1]{{\phon\textit{\mbox{#1}}}} %API tjs en italique
...
There is in addition one case of backformation:\footnote{One anonymous reviewers commented that all Stem alternations in words of Tibetic origin are backformations; I disagree in the sense that in Japhug, Stem 1 $\rightarrow$ Stem 3 alternation is regular and unidirectional, and thus only creating a Stem 1 from the Stem 3 counts as backformation. Note that a similar case is found with the native verb \ipa{kʰo} `give, pass, hand over', Stem 3 \ipa{kʰɤm}, which used to be a non-alternating verb, but whose Stem 1 was created by applying backwards the \ipa{-o} $\rightarrow$ \ipa{-ɤm} alternation (Jacques, \citeyear{jacques14esquisse}).} the form \ipa{fkrɤm} `put in order' borrowed from the Tibetan past stem \ipa{bkram} `arrange, set' is reanalyzed by some speakers as an alternating Stem 3, and these speakers create an innovative Stem 1 form \ipa{fkro}. 

 
\subsection{Denominal verbs}\label{sec:denominal}
Japhug has a very rich system of denominal prefixes (Jacques, \citeyear{jacques12incorp, jacques14antipassive}). Nouns of Tibetic or Chinese origin borrowed into Japhug can undergo these derivations and be turned into verbs, as shown by the examples in Table \ref{tab:denominal}.
...
All Tibetic languages have a more or less elaborate system of honorific verbs and nouns (see for instance Hill, \citeyear{hill08moriendi} for a detailed case study in Old Tibetan). Honorific words and their non-honorific counterparts are generally suppletive, though there is some evidence for a non-productive honorific *\ipa{-j-} infix (Gong, \citeyear{gong77}).
...
Although there is no person or number indexation in any Tibetic language, the second person plural pronoun \ipa{kʰʲed} becomes used as a second person singular honorific after the Old Tibetan period (see Hill, \citeyear{hill10pronouns}: 563-4). It cannot therefore be excluded that the honorific use of the plural in Japhug arose under the influence of Tibetic languages, first with the pronouns and then on verbal indexation.
...
\bibitem[{DeLancey(1981)}]{delancey81direction}
DeLancey, Scott. 1981.
\newblock {T}he category of direction in {T}ibeto-{B}urman.
\newblock \emph{Linguistics of the Tibeto-Burman Area} 6(1): 83--101.

\bibitem[{Gong(1977)}]{gong77}
Gong, Hwang-cherng. 1977.
\newblock {G}uzangwen de y jiqi xiangguan wenti
  \zh{古藏文的y及其相關問題} ({O}ld {T}ibetan y and related
  problems).
\newblock \emph{Bulletin of the Institute of History and Philology} 48.2.
  205--228.

\bibitem[{Gong(2014)}]{gongxun14agreement}
Gong, Xun. 2014.
\newblock {P}ersonal agreement system of {Z}bu r{G}yalrong ({N}gyaltsu
  variety).
\newblock \emph{Transactions of the Philological Society} 112(1): 44--60.

\bibitem[{Grossmann(under review)}]{grossmann16contact}
Grossmann, Eitan. under review.
\newblock {L}anguage-specific transitivities in contact: the case of {C}optic.
\newblock \emph{Journal of Language Contact} .

\bibitem[{Haller(2004)}]{haller04themchen}
Haller, Felix. 2004.
\newblock \emph{{D}ialekt und {E}rzählungen von {T}hemchen}.
\newblock Bonn: VGH Wissenschaftsverlag.

\bibitem[{Haspelmath(2011)}]{haspelmath11SAPTR}
Haspelmath, Martin. 2011.
\newblock {O}n {S}, {A}, {P}, {T}, and {R} as comparative concepts for
  alignment typology.
\newblock \emph{Linguistic Typology} 15.3: 535--567.

\bibitem[{Hill(2008)}]{hill08moriendi}
Hill, Nathan~W. 2008.
\newblock {V}erba {M}oriendi in the {O}ld {T}ibetan {A}nnals.
...
\bibitem[{Hill(2010{\natexlab{b}})}]{hill10pronouns}
Hill, Nathan~W. 2010{\natexlab{b}}.
\newblock {P}ersonal {P}ronouns in {O}ld {T}ibetan.
\newblock \emph{Journal Asiatique} 298.2: 549--571.

\bibitem[{Hill(2012)}]{hill12bas}
Hill, Nathan~W. 2012.
\newblock {T}ibetan -las, -nas, and -bas.
\newblock \emph{Cahiers de Linguistique Asie Orientale} 41.1: 3--38.

\bibitem[{Hill(2013)}]{hill13laterals}
Hill, Nathan~W. 2013.
\newblock {R}elative {O}rdering of {T}ibetan {S}ound {C}hanges {A}ffecting
  {L}aterals.
...
\bibitem[{Jacques(2013{\natexlab{b}})}]{jacques13harmonization}
Jacques, Guillaume. 2013{\natexlab{b}}.
...
\bibitem[{Michailovsky et~al.(2014)Michailovsky, Mazaudon, Michaud, Guillaume,
  François and Adamou}]{michailovsky14pangloss}
...
\bibitem[{Paul(2010)}]{paul10adj}
Paul, Walraud. 2010.
\newblock {A}djectives in {M}andarin {C}hinese: {T}he rehabilitation of a much
  ostracized category.
...
\bibitem[{Sagart and Baxter(2012)}]{sagart12sprefix}
Sagart, Laurent and William~H. Baxter. 2012.
\newblock {R}econstructing the s- prefix in {O}ld {C}hinese.
\newblock \emph{Language and Linguistics} 13.1: 29--59.

\bibitem[{Sun(1998)}]{jackson98morphology}
Sun, Jackson T.-S. 1998.
\newblock {N}ominal {M}orphology in {C}aodeng r{G}yalrong.
\newblock \emph{Bulletin of the Institute of History and Philology} 69(1).
  103--149.

\bibitem[{Sun(2000)}]{jackson00puxi}
...
\bibitem[{Sun(2012)}]{sun12complementation}
Sun, Jackson T.-S. 2012.
\newblock {C}omplementation in {C}aodeng r{G}yalrong.
\newblock \emph{Language and Linguistics} 13(3): 471--498.

\bibitem[{Sun and Shidanluo(2002)}]{jackson02rentongdengdi}
Sun, Jackson T.-S. and Shidanluo. 2002.
\newblock {C}aodeng {J}iarongyu yu rentong dengdi xiangguan de yufa xianxiang
  \zh{草登嘉戎語與「認同等第」相關的語法現象} ({E}mpathy
  {H}ierarchy in {C}aodeng r{G}yalrong grammar).
\newblock \emph{Language and Linguistics} 3(1): 79--99.

\bibitem[{Tournadre(1991)}]{tournadre91}
Tournadre, Nicolas. 1991.
\newblock {T}he rhetorical use of the {T}ibetan ergative.
\newblock \emph{Linguistics of the Tibeto Burman Area} .

\bibitem[{Tournadre(1996)}]{tournadre96erg}
Tournadre, Nicolas. 1996.
\newblock \emph{{L}'ergativité en tibétain}.
\newblock Louvain: Péeters.

\bibitem[{Tournadre(2010)}]{tournadre10cases}
Tournadre, Nicolas. 2010.
\newblock {T}he {C}lassical {T}ibetan cases and their transcategoriality,
...
\bibitem[{Zeisler(2004)}]{zeisler04}
