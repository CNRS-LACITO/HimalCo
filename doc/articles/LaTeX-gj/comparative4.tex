...
%Anton Antonov, Giorgio Arcodia, Lauren Gawne, Aimée Lahaussois, Alexis Michaud,  Enrique Palancar, Mathieu Ségui, Amos Teo, Nicolas Tournadre, Fernando Zúñiga, +2 anonymous     +editors

%\author{Guillaume JACQUES}
\maketitle
   \sloppy
\textbf{Abstract}: This paper describes the uses of the marker  \ipa{kɯ} in Japhug, which presents many distinct functions, including ergative, instrumental, distributive, causal linker, manner linker, emphatic adversative, interrogative sentence final particle and comparee marker in the comparative construction. A series of grammaticalization pathways, some of which have never been documented before, are proposed to account for the polyfunctionality of this marker.
...
The agent is relativized in prenominal or head-internal relative clauses with the main verb in the S/A-participle form (marked by the prefix  \ipa{kɯ--}) with an  additional possessive prefix coreferent with the P, as \ipa{ɯ--} in \ipa{ɯ-kɯ-nɯmbrɤpɯ} `the one who rides it' in \ref{ex:WkWnWmbrApW}.
...
On the other hand, the instrument is relativized in prenominal relative clauses with the verb in the oblique participle form (with the prefix \ipa{sɤ}--). The  causative prefix \ipa{sɯ}-- is also removed in this form, as illustrated by example \ref{ex:sAxtCAr}.  
...
\end{exe}
  ...
  It also appears in a clausal construction with the possessed noun `its strength' \ipa{ɯ-xɕɤt} borrowed from Tibetan \ipa{ɕed} `strength'.\footnote{This borrowing comes specifically from Amdo, as it reflect an intermediate stage in the sound change from [\ipa{ɕet}] to [\ipa{xɕit}] as in most Amdo varieties.}
...
\glt `Because of the pain, I felt as though my skull had cracked.' (24 pGArtsAG, 77)
  \end{exe}
...
Another causal construction in \ipa{kɯ} involves the noun \ipa{tʰɯrʑi} `mercy' (from Tibetan \ipa{tʰugs.rdʑe}), as in example \ref{ex:WthWrZi.kW}.
...
% Outside of its use as agent, instrumental or causal marker, \ipa{kɯ} is also selected by the transitive verb \ipa{kʰɤt} ``to do over a long time", where an abstract noun or an action nominalization with the ergative indicates the realized action.
% \begin{exe} 
%\ex \label{ex:instr5}
...
\end{exe}
 ...
 The marker \ipa{kɯ} is not restricted to the multiclausal nominalized degree construction, as it also occurs optionally with the monoclausal one, as in \ref{ex:kW.pWsaXaR}, although this use is very rare.
...
commerce \textsc{inf}-make \textsc{3sg-nmlz:degree}-can \textsc{inf}-say ?{ } \textsc{pst.ipfv}-be.extremely \textsc{emph} \textsc{sens}-be \\
...
%\textsc{3sg.poss}-leaf \textsc{dem} \textsc{pl}  \textsc{neg-sens}-be.round ?{ } \textsc{sens}-be.long little \\
