...
\gll 
  ...
  \textsc{3sg.poss}-ear \textsc{3sg.poss}-one.of.a.pair \textsc{dem} \textsc{erg?} \textsc{nmlz:S/A-redp}-be.small \textsc{sens}-be.well but \textsc{dem} also much \textsc{neg:sens}-be.good \\
...
\gll 
  ...
  chicken \textsc{3sg.poss}-body about exist:\textsc{sens} but \textsc{dem} as.much \textsc{nmlz:S/A-redp}-be.small \textsc{neg:sens}-be.big \\
...
\subsubsection{Independent words}
The only non-grammatical element with a shape resembling \ipa{kɯ} in Japhug is \ipa{---kɯ}  `east', found in locative nouns and adverbs such as \ipa{akɯ} and  \ipa{tɕɤkɯ} `in the east'. Although grammaticalization of ergative, distributive, and causal  markers  from spatial cases has been documented, it is very unlikely that any of the functions of \ipa{kɯ} described in section  \ref{sec:synchronic} can be shown to derive from this element. 

XXXX

The sentence final particle \ipa{kɯ} is used in rhetorical or  introspective questions (\ref{ex:pWwGsat}).  
...
probably cognate with Tangut \ipa{kjɨ} (see \citealt{jacques11tangut.verb})
...
Japhug, like other Rgyalrongic languages, has a mainly prefixing typology, with very few suffixes (\citealt{jacques13harmonization}).  There are strong phonological constraints on the possible shapes of the prefixes: with the exception of a few directional prefixes of recent origin, prefixes may not contain (i)  stops other than voiceless unaspirated stops (ii) consonant clusters (iii) vowels other than \ipa{a}, \ipa{ɤ} and \ipa{ɯ}  (\citealt{jacques14antipassive}).  
 ...
 As a consequence of these constraints, many homophonous prefixes with shapes such as consonant+\ipa{ɯ--}  are found in the language; for instance, no less than six  unrelated prefixes have the shape \ipa{nɯ--}.There are four prefixes with the \ipa{kɯ--}, as indicated in Table \ref{tab:homophones.kW}. 
...
All \ipa{kɯ--} prefixes can be shown to derive historically from the function of S/A participle (see \citealt{jacques15generic}).  
...
\citealt[95-6]{benedict72}
 
\citealt{delancey84case}
\citealt{lapolla95ergative}

Japhug preserves final \ipa{--z} in the inherited and even in the oldest layers of the borrowed vocabulary, \ipa{ʁnɯz} `two'  vs. Tibetan \ipa{gɲis} `two'.
...
\textsc{instrumental} $\rightarrow$ \textsc{cause}
\textsc{cause} $\rightarrow$ \textsc{infinitival manner}

  polyfunctionality already present in Tibetan before the marker was borrowed
