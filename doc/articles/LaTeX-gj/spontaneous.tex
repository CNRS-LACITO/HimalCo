...
An extension of the spontaneous value of the prefix \ipa{nɯ--} is the meaning `casually', `at one's will' (Chinese \zh{随便}), as in the following examples:

"hua guniang" ra tu-nɯ-ti-nɯ ɲɯ-ŋu.
nɯnɯra ʑara kɯ ɯ-rmi tu-nɯ-tɕɤt-nɯ ɲɯ-ŋu. （这是村民他们随便取的名字）


This particular value of the prefix \ipa{nɯ--}  explains its frequent presence in the protasis of alternative  (\ref{ex:pannWri}) and scalar (\ref{ex:pWnnWtu.kWnA}) concessive conditionals (see \citet{jacques14linking}.
...
In these constructions, the result described in the apodosis takes place regardless of whether the condition in the protasis is fulfilled or not. It is a particular instance of this XXX
...
No nasal prefix with a semantics comparable to Japhug \ipa{nɯ--} has been described in any other Sino-Tibetan language. Hence, it is likely that it is one of the many common innovations of the Rgyalrongic languages.\footnote{However, one cannot completely exclude the possibility that has been lostwithour traces in some languages.} 

This raises the question of the source of the prefix \ipa{nɯ--}and of its historical relantionships to phonetically similar derivations in Japhug. While the ultimate origin of the autobenefactive \ipa{nɯ--}is not yet clear, we show that it is likely to be related to two other derivations: the vertitive and the anticausative.

%\footnote{However, no trace of this prefix has been found in Stau up to now.}


\subsection{Vertitive}
There are three derivational verbal prefixes homophonous with the spontaneous-autobenefactive (\citealt{jacques13tropative}): the applicative \ipa{nɯ--} / \ipa{nɯɣ--}, the denominal \ipa{nɯ--} and the vertitive \ipa{nɯ--}. The first two are unlikely to be historically related to the autobenefactive. In particular, while other voice derivations, including the antipassive \ipa{rɤ--}, the applicative \ipa{nɯ--} and the causative \ipa{sɯ--} have been shown to derive from denominal derivations (see \citealt{jacques14antipassive}), the autobenefactive \ipa{nɯ--} and the denominal derivations in \ipa{nɯ--} are semantically too different for such a hypothesis to be possible: there are no denominal verbs in \ipa{nɯ--}  in Japhug with an intrinsic spontaneous or autobenfactive meaning. Therefore, this section only focuses on the vertitive.
...
All known examples of anticausative alternations are presented in Table \ref{tab:anticausative}.
...
The anticausative is widespread throughout the Sino-Tibetan family


Although some authors appear to  confuse the anticausative and causative \ipa{s--} derivations (\citet{mei12caus}), the general consensus is that these are completely distinct phenomena (\citealt{lapolla03}, \citealt{jacques12internal}, \citealt{sagart12sprefix}, \citealt{hill14voicing}).
