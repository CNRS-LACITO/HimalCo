...
The paradigm of regular numeral prefixes from 1 to 10 in Kamnyu Japhug is indicated in Table \ref{tab:num.prefix.1.to.10}. The prefixed are derived from the corresponding numeral by \textit{status constructus} (§ \ref{sec:status.constructus}), with loss of the coda and vowel alternation. Vowel alternation is optional for \japhug{kɯβde}{four} and  \japhug{kɯmŋu}{five}. The alternation between \japhug{tɤɣ}{one} and \forme{tɯ-} is irregular. The form \forme{tɤ-}, which is otherwise attested in another  paradigm (Table \ref{tab:num.prefix.tArZaR}), should be the regular form. In the corpus,  the coda \forme{-z} of the numeral \japhug{kɯɕnɯz}{seven} seems to be preserved in one example (\ref{ex:kWCnWztWphu}), but this is an incorrect form, with a slight pause of hesitation between the numeral of the following noun.
...
In addition to the numerals mentioned above, some nouns can appear as prefixes of counted nouns, in particular the numerals above 99 and the participle of the stative verb \japhug{antɕʰɯ}{be many}. XXXXX

The irregular noun \japhug{tɤ-rʑaʁ} {one night} has an alternative paradigm with \ipa{ɤ} instead of \forme{ɯ} in the prefixes (see Table \ref{tab:num.prefix.tArZaR}). \forme{χsɤ-tɣa} XXX
