...
The postposition \japhug{ɕɯŋgɯ}{before} (§ \ref{sec:temporal.postpositions}) occurs after time CNs in examples such as (\ref{ex:sqamNusNi.CWNgW}); the relator noun \japhug{ɯ-qʰu}{after} (§ XXX) is not used in this way except in texts translated from Chinese, where it is likely calqued as in (\ref{ex:kWBdexpa.Wqhu}).
...
\japhug{tɯ-sŋi}{one day} & \japhug{sɲikɯku}{every day} \\
%\japhug{tɯ-sla}{one month} & \japhug{slɤrɯri}{every months} \\
\japhug{tɯ-xpa}{one year} & \japhug{pakɯku}{every year} \\
\lspbottomrule
\end{tabular}
\end{table}
