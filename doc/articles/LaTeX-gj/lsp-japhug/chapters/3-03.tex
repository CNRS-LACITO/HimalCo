...
Most deverbal CNs are from transitive verbs, but examples from intransitive verbs such as \japhug{tɯ-tsʰoz}{one complete set} from the stative intransitive verb \japhug{tsʰoz}{be complete} are also found. XXXX

%nɯnɯ kɯ iɕqʰa rŋɯl kɯ tɯ-ŋga thɯ-kɤ-βzu ci tɯ-tsʰoz pjɤ-βde.

Deverbal CNs either designate a quantity of objects resulting from the action of the verb (for instance \japhug{tɯ-fkur}{one load} or \japhug{tɯ-tɤrtsɯɣ}{one pile} from \japhug{rtsɯɣ}{pile up} as in \ref{ex:RnWtArtsWG}) or the number of time an iterative/semelfactive action takes place (such as \japhug{tɯ-tɤtɕʰɯ}{hitting with the hoe one time} from \japhug{tɕʰɯ}{hit, gore}).
...
CNs are used for various type of measures, including size and weight (measures of time are discussed in § \ref{sec:time}). Although Chinese words, or calques from Chinese often occur in Japhug, parts of the former systems are still in current use.
