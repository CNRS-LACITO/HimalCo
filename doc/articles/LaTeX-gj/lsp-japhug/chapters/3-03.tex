...
\subsubsection{Partitive numeral prefixes} \label{sec:numeral.prefixes.partitive}
The partitive form of CNs is built by reduplicating the numeral `one' prefix \forme{tɯ-}, as \forme{tɯ-tɯ-rdoʁ} from the generic CN \japhug{tɯ-rdoʁ}{one piece} (see § \ref{sec:CN.classification}) in example (\ref{ex:tWtWrdoR}).
...
\subsubsection{Partitive use of CNs}
%tɕeri tɯ-tɯ-rdoʁ tɯ-tɯ-rdoʁ ɲɯ-ŋu ma kɯ-ɤndʑɯrɣa kɯ-fse kɯ-ɤrɤkhɯmkhɤl kɯ-fse maŋe. 
\subsection{CNs and semantic classes} \label{sec:CN.classification}
\subsection{CNs and IPNs}  

The IPN \japhug{ɯ-mdoʁ}{colour}  (from Tibetan \tibet{མདོག་}{mdog}{colour}) derives a CN with the numeral prefix \forme{kɤntɕʰɯ-} `many' (on which see § \ref{sec:other.numeral.prefixes}) in (\ref{ex:kAntChWmdoR}). %Augenblickbildung?
