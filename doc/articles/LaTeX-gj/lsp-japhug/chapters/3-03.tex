...
The numerals for decades (Table \ref{tab:decades}) are relatively straightforward. With the exception of \japhug{ɣnɤsqi}{twenty} and \japhug{fsɯsqi}{thirty}, they are predictable by combining \japhug{sqi}{ten} to the corresponding numeral prefix (§ \ref{sec:numeral.prefixes}). The numeral \japhug{kɯngɯsqi}{ninety} is ambiguous, as the same form can also mean `nine or ten' (see Table \ref{tab:approx.num.1to10}).
...
\gll <tuolaji> tɯ-ɣjɤn ju-ɣɯt nɯ rcanɯ, ɣnɤsqi fsɯsqi-fkur jamar ju-sɯ-ɤzɣɯt cʰa.  \\
 tractor one-time \textsc{ipfv}-bring \textsc{dem} \textsc{foc}:\textsc{unexp} twenty thirty-burden about \textsc{ipfv}-\textsc{caus}-reach can:\textsc{fact} \\
...
people two \textsc{ifr}.\textsc{ipfv}-exist \textsc{lnk} people one-piece \textsc{dem} \textsc{foc}:\textsc{unexp} \textsc{3sg}.\textsc{poss}-truth completely \textsc{emph} \textsc{ipfv}-do[III] \textsc{lnk} \\
...
ICNs can also express a combination of partitive and restrictive meaning, `just X of them', as in (\ref{ex:tWrdoR.mapWwGsat}).
...
However, the use of ICN without partitive, distributive or restricted meaning are not unattested as in  (\ref{ex:xCAndZu.XsWldZa}), where the ICN \japhug{χsɯ-ldʑa}{three long objects} occurs in the same context as the numerals \japhug{ci}{one} and \japhug{χsɯm}{three}. In (\ref{ex:si.tWphW.pjAtu}) likewise, it is clear from the context that neither a partitive, distributive nor restrictive interpretation is possible.
...
Such use is not very widespread in native texts and appears to occur mainly with nouns selecting ICNs other than \japhug{tɯ-rdoʁ}{one piece} (see § \ref{sec:CN.classification}).
...
When no other numeral or CN is present, the modifier of Tibetan origin \japhug{raŋri}{each} (from \tibet{རང་རེ་}{raŋ.re}{each}; see also § \ref{sec:raNri}) can be used to specific the distributive meaning as in (\ref{ex:tWtWpW.rANri}).
...
Iterative/Semelfactive CNs (henceforth SCNs) designate the number of time that a bounded action takes place. This group includes generic CNs derived from verbs (§ \ref{sec:CN.verbs})  such as \japhug{tɯ-tɤtɕʰɯ}{hitting with the hoe one time} (from \japhug{tɕʰɯ}{hit, gore}) and \japhug{tɯ-tɤfskɤr}{one turn} (from \japhug{fskɤr}{turn around}) or from ideophones  (§ \ref{sec:CN.ideophones}) such as \japhug{tɯ-tɤxɯr}{one turn}.

SCNs can either be used as objects of an auxiliary verb, mainly \japhug{lɤt}{throw}, as in (\ref{ex:kAntChWtAtChW}) and (\ref{ex:RnWtAxWr}), or as sentential quantifiers (§ \ref{sec:sentential.CN}), as in (\ref{ex:laRnWtAxWr}) with the intransitive verb \japhug{mtɕɯr}{turn}.
...
However, in a text translated from Chinese, we do find a calque of the Chinese construction (\ch{我一个人}{wǒ yīgèrén}{I alone}) with the pronoun \japhug{aʑo}{\textsc{1sg}} follow by the CN \japhug{tɯ-rdoʁ}{one piece}; this sentence is unidiomatic, but not radically agrammatical.
...
A minority of nouns select other ICNs referring to specific shapes: \japhug{tɯ-ldʑa}{one long object}, \japhug{tɯ-mpɕar}{one leaf} and \japhug{tɯ-pʰɯ}{one tree}.

 
The ICN \japhug{tɯ-ldʑa}{one long object}, like Chinese \ch{一条}{yitiáo}{one long object} or \ch{一根}{yigēn}{one long object}, occurs in the corpus with nouns belonging to the following semantic categories:
...
The ICN \japhug{tɯ-mpɕar}{one leaf} occurs with flat objects, including tree leaves (with \japhug{tɤ-jwaʁ}{leaf} as in \ref{ex:RnWmpCar}), sheets of cloth (with \japhug{raz}{cloth}) and snowflakes (with \japhug{tɤjpa}{snow}).  
...
The CN \japhug{tɯ-mpɕar}{one leaf} can also refer to money (in present-day China, bank notes are by far more common than coins even for small amounts of money), meaning `one renminbi'.

The CN \japhug{tɯ-phɯ}{one tree} occurs with the generic noun \japhug{si}{tree} or names of particular species.

\subsubsection{CCN}
Most nouns referring to humans and animals can occur with the CN \japhug{tɯ-boʁ}{one group}. However, the CCN \japhug{tɯ-ɟɯɣ}{one pack} appears to be exclusively used with horses. Its only attestations in the corpus (for instance \ref{ex:tWJWG}) are found in a translated text.
...
\subsubsection{Multiple CNs}
Some nouns are compatible with more than one CN postnominal modifier, with different connotations. For instance \japhug{zgo}{mountain} can be used with the generic CN \japhug{tɯ-rdoʁ}{one piece} as in (\ref{ex:zgo.tWrdoR}), but also with  \japhug{tɯ-ldʑa}{one long object}  as in (\ref{ex:zgo.RnWldZa}) in the meaning `mountain range' and also with \japhug{tɯ-tɤɣmbaj}{one side} to mean `mountain face'.
...
In other cases, the change of CN does not entail a radical semantic contrast. For instance, the noun \japhug{si}{tree}, while generally used with \japhug{tɯ-pʰɯ}{one tree}, is also attested with the CN \japhug{tɯ-ldʑa}{one long object}, as in (\ref{ex:si.tWldZa}), without clear semantic difference.

\begin{exe}
\ex \label{ex:si.tWldZa}
\gll maka rdɤstaʁ cʰo si rcanɯ tɯ-ldʑa cinɤ ʑo kɯ-me scʰiz kɤ-azɣɯt-ndʑi ɲɯ-ŋu, \\
completely stone \textsc{comit} tree \textsc{foc}:\textsc{unexp} one-long.object even.one \textsc{emph} \textsc{nmlz}:S/A-not.exist \textsc{indef}.\textsc{loc} \textsc{pfv}:\textsc{east}-reach-\textsc{du} \textsc{sens}-be  \\
...
The derivation process is quite productive, and potentially new CNs meaning `a ... full of' can be derived from any APN if appropriate, as \forme{tɯ-co} from \japhug{co}{valley} in (\ref{ex:mbro.tWco}), which means in this particular context `an entire valley full of ...'.
...
There also are more lexicalized and synchronically less obvious examples of CNs derived from verbs. For instance the time CN \japhug{tɯ-xpa}{one year} originates from the intransitive verb \japhug{pa}{pass X years}  illustrated in (\ref{ex:12.topa}) (see also example \ref{ex:40.topa}  in § \ref {sec:uses.numerals} and the account of the \forme{-x-} element in \forme{tɯ-\textbf{x}pa} proposed in in § \ref{sec:num.prefix.paradigm.history}). 
...
The collective CNs \japhug{tɯ-boʁ}{one group} and \japhug{tɯ-ɟɯɣ}{one pack} respectively originate from the ideophonic roots found in \japhug{boʁboʁ}{in group, in order} and \japhug{ɟɯɣɟɯɣ}{in great number (of long objects)} -- the presence of plain voiced initial stops in a clue to their ideophonic origin (§ XXX).

The semelfactive/iterative CN \japhug{tɯ-tɤxɯr}{one turn} comes from the ideophonic root of \japhug{xɯrxɯr}{round} (pattern III \japhug{xɯrnɤxɯr}{turning around}) with an additional prefix \forme{tɤ-}. This prefix is possibly the trace of an intermediate stage as an IPN, with a two-step derivation \textsc{ideophone} $\rightarrow$ IPN $\rightarrow$ CN.

Direct derivation from ideophone to SCN is also attested, for instance \japhug{tɯ-ɬɯm}{one period of sleep} from the root in \japhug{ɬɯmɬɯm}{have a nice sleep} without the \forme{tɤ-} prefix.
...
Japhug has CNs for the following precise time durations: \japhug{tɯ-xpa}{one year}, \japhug{tɯ-sla}{one month}, \japhug{tɯ-sŋi}{one day}, \japhug{tɤ-rʑaʁ}{one night} (also used to express 24 hours) and \japhug{tɯ-skɤrma}{one minute} (the latter one from Tibetan \tibet{སྐར་མ་}{skar.ma}{star, minute}). There are no native concepts for `weeks' or `decades'; the expression of hours is presented in § \ref{sec:hours}. Other time CNs include \japhug{tɯ-mɲɯtsi}{one lifetime} (from \tibet{མི་ཚེ་}{mi.tsʰe}{human life}), \japhug{tɯ-tɯpɕɯrtɕʰaʁ}{one generation} and \japhug{tɯ-rzɯɣ}{one section, one instant}.


The time CNs  \japhug{tɯ-sŋi}{one day} and \japhug{tɤ-rʑaʁ}{one night} are commonly used in apposition with the same numeral prefix to express the meaning `X days and X nights', as in (\ref{ex:XsWsNi.XsArZaR}).
...
To express the meanings corresponding to English `after' or `later' with a time span, the idiomatic way in Japhug is to use verbs such as \japhug{tsu}{pass}, as shown by example (\ref{ex:tWsNi.tAtsu}) and (\ref{ex:tAtsua}). Note that \japhug{tsu}{pass} is a semi-transitive verb (§ XXX) whose semi-object is the time period expressed by the CN, and whose subject is the person affected by the passing of time (see for instance \ref{ex:tAtsua} with \textsc{1sg} indexation).
...
The postposition \japhug{ɕɯŋgɯ}{before} (§ \ref{sec:temporal.postpositions}) occurs after time CNs in examples such as (\ref{ex:sqamNusNi.CWNgW}); the relator noun \japhug{ɯ-qʰu}{after} (§ \ref{sec:relator.temporal}) is not used in this way except in texts translated from Chinese, where it is likely calqued as in (\ref{ex:kWBdexpa.Wqhu}).
...
In order to express meanings such as `beginning' or `end' of a time period indicated by a CN, verbs such as \japhug{arɕo}{be finished} are used as in (\ref{ex:tWxpa.YArCo}) instead of nouns or participles like \japhug{ɯ-sɤɣjɤɣ}{its end}.
...
Unlike ICNs and CCNs, Time CNs are generally used one their own without head noun. The CN \japhug{tɯ-xpa}{one year} however does occur with the noun \japhug{lu}{year} (from Tibetan \tibet{ལོ་}{lo}{year}) in some traditional stories as in (\ref{ex:lu.XsWxpa}).
...
The day ordinals from $D^{+2}$ to $D^{+5}$ contain a suffix \forme{-ndi}, which attaches to the \textit{status constructus} forms of \japhug{fso}{tomorrow} in $D^{+2}$ \forme{fsɤndi} and \japhug{ɯ-qʰu}{after} in $D^{+3}$ \forme{qʰɤndi}. The $D^{+5}$ form \forme{βʑindi} (homophonous with the noun for `leggings') contains the \textit{status constructus} \forme{βʑɯ-} of the Japhug pronunciation \forme{βʑi} of the Tibetan numeral \tibet{བཞི་}{bʑi}{four}  (see § \ref{sec:one.to.ten}), presumably meaning `the fourth day after tomorrow'.
...
Note the isolated \japhug{qʰuj}{this afternoon}, which can refer to the time period between noon and the night (including the early evening). Its etymology is discussed in § XXX.
...
Japhug has a series of distributive adverbs meaning `every/each X' derived from time nominals (mainly CNs, but also APNs like \japhug{soz}{morning}) by adding a suffix \forme{-ku} and then applying partial reduplication (§ XXX). If the nominal root has a coda, it is resyllabified and undergoes reduplication together with the suffix, as in the form \japhug{mɯrkɯrku}{every evening} (not $\dagger$\forme{mɯrkɯku}). 

The \ipa{ɯ} of the preceding syllable tends to become [\forme{u}] due to vowel harmony (§ XXX); since vowel harmony always occurs in the forms without reduplicated cluster, the transcriptions \japhug{sɲikuku}{every day}  and  \japhug{pakuku}{every year}  are used in this grammar (Table \ref{tab:every.time}) instead of the more phonological representations \ipa{sɲikɯku} and \ipa{pakɯku}.
...
This morphological derivation is not productive, and cannot be used with any other time CN, even \japhug{tɯ-sla}{one month} -- the distributive modifiers \japhug{raŋri}{each, every} or \japhug{rɯri}{each, every} are used instead (§ \ref{sec:raNri}).
...
\gll  tɕe nɯnɯ ci ta-ɕɯ-mŋɤm nɯnɯ tɯtsʰot tɯ-rdoʁ jamar mɤɕtʂa mɤ-ʑi. \\
 \textsc{lnk} \textsc{dem} one \textsc{pfv}:3\fl3'-caus-hurt \textsc{dem} hour one-piece about until \textsc{neg}-subside:\textsc{fact} \\
