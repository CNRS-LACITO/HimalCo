...
The $D^{0}$ to $D^{-2}$ and $Y^{0}$ to $Y^{-2}$ordinals have a prefix of probably demonstrative origin \forme{ji-/jɯ-/ja-}, which also occurs under other forms in some frozen adverbs, such as \japhug{jɤxtsʰi}{this time}, and is found in other time ordinals elsewhere in Gyalrongic. In the case of \japhug{ɣɯjpa}{this year}, this prefix surfaces as a preinitial  \forme{-j-} to the root \forme{-pa} `year' (see § \ref{sec:num.prefix.paradigm.history} on its etymology). The \forme{ɣɯ-} (from \forme{*wə-}) prefix may be cognate to the prefix \forme{pə-} in \stau{pəvə}{this year} and \stau{pəsɲə}{today} in Stau.
...
However, in many cases, they have less specific meanings `a few days ago' or `a few years ago' ,  as shown by (\ref{ex:jWfCWndzxi}), referring to an event that had occurred more than one week before.
...
Year ordinals from $Y^{+2}$ to $Y^{+5}$ are built by simply adding the root \forme{-pa} `year' (see § \ref{sec:num.prefix.paradigm.history} and §  \ref{sec:CN.verbs}) to the \textit{stratus constructus} of the corresponding day ordinals $D^{+2}$ to $D^{+5}$.
