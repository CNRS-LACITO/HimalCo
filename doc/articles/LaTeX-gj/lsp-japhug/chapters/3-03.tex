...
The day ordinals from $D^{+2}$ to $D^{+5}$ contain a suffix \forme{-ndi}, which attaches to the \textit{status constructus} forms of \japhug{fso}{tomorrow} in $D^{+2}$ \forme{fsɤndi} and \japhug{ɯ-qʰu}{after} in $D^{+3}$ \forme{qʰɤndi}. The $D^{+5}$ form \forme{βʑɯndi} (homophonous with the noun for `leggings') contains the \textit{status constructus} \forme{βʑɯ-} of the Japhug pronunciation \forme{βʑi} of the Tibetan numeral \tibet{བཞི་}{bʑi}{four}  (see § \ref{sec:one.to.ten}), presumably meaning `the fourth day after tomorrow'.
