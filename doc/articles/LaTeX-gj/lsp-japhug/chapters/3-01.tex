...
The only exception in the corpus is (\ref{ex:XsAr.akWmtChW}), where the prenominal modifier \japhug{χsɤr}{gold} does appear before the possessive prefix \forme{a-}, but the alternative form \forme{a-χsɤr kɯmtɕʰɯ} is attested in the same text.
...
This section only addresses the morphosyntactic properties of proper names; inventories of proper names, and discussions on their morphological structure and etymology are found in sections § XXX and § XXX.
%schluecker17proper
\subsubsection{Personal names}  \label{sec:personal.names}
Speakers of Japhug generally have Tibetan names (given by a lama), and in addition a Chinese official name which may or may not be related to the Tibetan one (§ XXX).
...
\glt `There was (a lady) was was called `Mung bean', and (another one) called `flax'.' (zrAntCW, 1)
\end{exe}

% a-βɣo χpɤltɕin

\subsubsection{Place names}  \label{sec:place.names}
Place names (\japhug{mbarkʰom}{Mbarkham}, \japhug{kɤmɲɯ}{Kamnyu} etc) and names of ethnic groups (such as \japhug{kɯrɯ}{Tibetan, Gyalrong} or \japhug{kupa}{Chinese}) cannot take possessive prefixes when used independently. They can only be occur with independent pronouns as in (\ref{ex:jiphe.kAmYW}) or (\ref{ex:iZo.KamnYW}) below.
...
Like other locative nouns (§ XXX), bare placenames can be used without postposition to express motion (\ref{ex:mbarkhOm.thWwGGWta}) or static location (\ref{ex:kAmYW.GJW}), but are also found with locative postpositions (as in \ref{ex:jiphe.kAmYW} above).
...
First, some noun allow regular partial reduplication with \forme{ɯ} in the reduplicated syllable. XXXXX

Second, we find the reduplicated collectives with the vowel \ipa{a}, not \ipa{ɯ}, in the replicated syllable. Examples are few, as shown in Table \ref{tab:coll.n}, but several of them are borrowings from Tibetan.
