...
NIPN body parts are rare; examples include \japhug{qame}{mole}, with a \forme{qa-} class prefix (see section \ref{sec:class.prefixes}).
...
The property nouns \japhug{ɯ-do}{old one}  and \japhug{tɤ-mbe}{old thing} differ in that the former one is used for living things (including animals and plants), while the second occurs with inanimate objects.

Property nouns are not commonly used with an indefinite possessor prefix; in attested examples, it is always \forme{tɤ-}. They origins are diverse: \japhug{ɯ-pɯ}{little one} derives from \japhug{tɤ-pɯ}{offspring, young} (see \ref{sec:diminutive}), while \japhug{tɤ-mbe}{old thing} and \japhug{ɯ-do}{old thing}  originate  from \japhug{mbe}{be old} and \japhug{do}{be old (of palnts)} by deverbal derivation (section XXX). Some \forme{tɤ-} prefixed nouns of verbal origin like \japhug{tɤkʰe}{idiot, fool} (from \japhug{kʰe}{be stupid}) may come from former property nouns.
...
\subsection{Comparative perspectives} \label{sec:ipn.st}
...
Morphophonological alternations affecting second members of compounds are very rare in Japhug.

Several cases are found with animal nouns with the uvular class prefix \forme{qa-}, which has a variant \ipa{χ-/ʁ-} when occurring as second member of compounds (see \ref{sec:uvular.animal}).
...
Note that \japhug{ʁmbroŋ}{wild yak} is a borrowing from Tibetan \tibet{འབྲོང་}{ɴbroŋ}{wild yak}, a fact that possibly suggests that the \forme{χ-/ʁ-} prefix has some degree of productivity (see \citealt{jacques14snom}). 

The allomorph \forme{qa-} is reduced to its non-syllabic variants \forme{χ-/ʁ-} when the prefixed noun occurs as second member of compound. The nouns \japhug{tɕʰɯχpri}{salamander} and \japhug{rtɕʰɯrjɯ}{caterpillar} are examples of this reduction. The former is a compound of \forme{tɕʰɯ-} (a syllable borrowed  from Tibetan \tibet{ཆུ་}{tɕʰu}{water}) and \forme{-χpri}, a variant of \japhug{qapri}{snake}. The latter comprises the syllable \forme{rtɕʰɯ-}, \textit{status constructus} of the unprefixed stem of \japhug{tɯrtɕʰi}{type of vegetable (\zh{酸酸菜})}, and the second \forme{-ʁjɯ} is the reduced variant of \japhug{qajɯ}{worm}.

\begin{table}
\caption{Animal name \forme{qa-} prefix} \label{tab:animal.qa}
\begin{tabular}{l|l}
 \lsptoprule 
 \japhug{qacʰɣa}{fox} 	&	\japhug{qandʐi}{anadromous fish} 	\\
\japhug{qaɕpa}{frog} 	&	\japhug{qandʑɣi}{falcon} 	\\
\japhug{qajɯ}{worm} 	&	\japhug{qaɲi}{mole} 	\\
\japhug{qaɟy}{fish} 	&	\japhug{qapar}{dhole} 	\\
\japhug{qala}{rabbit} 	&	\japhug{qapri}{snake} 	\\
\japhug{qaliaʁ}{eagle} 	&	\japhug{qarma}{crossoptilon} 	\\
\japhug{qambrɯ}{male yak} 	&	\japhug{qartsʰaz}{deer} 	\\
\japhug{qandʐe}{earthworm} 	&	\japhug{qaʑo}{sheep} 	\\
%\japhug{qacʰɣa}{fox} \\
%\japhug{qaɕpa}{frog} \\
%\japhug{qajɯ}{worm} \\
%\japhug{qaɟy}{fish} \\
...
\glt `Its twigs cannot grow very long, and look like they have been sawedshort, therefore it is called `headless willow'. `Headless' means `without head'.'(07-Zmbri, 34-36)
\end{exe}

\begin{exe}
\ex \label{ex:jmAlu}
\gll tɕe kɯju jmɤlu nɯnɯ tɯrme ɲɯ-ŋu, ɯ-jme kɯ-me nɯ tɕe, tɕe kɯju jmɤlu nɯnɯ ɲɯ-sɲu ɕti tɕe nɯ  nɯ-sɲu tɕe tɕe iɕqha tɯ-rɣi cʰɯ-kɯ-χtɤr nɯ nɯ-kɯ-sɲu tu-sɤrmi-nɯ. \\
...
\glt `The `tailless animal' is the man, and `he becomes crazy', (the crows) call `crazy' the ones who sow seeds.' (22-qajdo, 47-9)
\end{exe}
...
The suffixes \forme{-do} and \forme{-mbe}, like their corresponding property nouns, differ in that the former occurs with animals and plants (\japhug{nɯŋa-do}{old cow}, \japhug{rɟɤlpu-do}{old king}), while the latter is used for inanimate objects.
 ...
 In a few cases, the suffixed noun is in status constructus (as \japhug{kʰɤɴqra}{ruin} from \japhug{kʰa}{house} and \forme{-ɴqra}). When the suffixed noun is an IPN, addition of a derogative suffix can turn it into a NIPN with loss of the possessive prefix, as \japhug{rcɤmbe}{old jacket} from \japhug{tɯ-rcu}{jacket} and \forme{-mbe}.
