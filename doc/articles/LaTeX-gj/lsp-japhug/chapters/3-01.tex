...
Given the existence of such forms, personal names are treated as a subclass of APN rather than as UPNs. Note that only plural forms (\forme{ji-χpɤltɕin} `our Dpalcan', \forme{ʑara nɯ-χpɤltɕin} `Their Dpalcan' etc) are possible; singular forms such as $\dagger$\japhug{a-χpɤltɕin} are not grammatical.
...
\glt `Dpalcan's house is called Tarrdo, so one (can) call him `Tarrdo Dpalcan'.' (elicited)
\end{exe}

If  two persons from the same household have the same name, locational modifiers (§ XXX) can be used instead, as illustrated in (\ref{ex:taRrdo.dpalcan}).
