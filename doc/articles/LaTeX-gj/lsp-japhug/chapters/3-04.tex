...
The additive and scalar focus marker \japhug{kɯnɤ}{also, even} always immediately follows the constituent over which it has scope, which can be noun phrases, postpositional phrases but also subordinate clauses (these are treated in § XXX). 
...
It is found with nouns or pronouns in core argument function, including S (\ref{ex:kWnA.nArca}), O (\ref{ex:nWXpWm.kWnA}), and semi-objects (\ref{ex:kWnA.mAsna}).  Examples with transitive subjects are presented below (\ref{ex:nWra.kWnA} and \ref{ex:Wzda.ra.kWnA}).
...
It also occurs with all types of oblique arguments and adjuncts, including genitive (\ref{ex:aZWG.kWnA}), dative (\forme{ɯ-ɕki} \ref{ex:nWCki.kWnA}),  locational adjuncts (\ref{ex:kutCu.kWnA}) or temporal adjuncts (\ref{ex:ftCAXcAl.kWnA}). In (\ref{ex:aZWG.kWnA}), note that \forme{kɯnɤ} occurs between the genitive pronoun and the following noun, which bears a \textsc{1sg} possessive prefix \forme{a-} coreferent with that pronoun.
...
%    tɕe nɯtɕu kɯnɤ ɯ-jaʁ ɯ-ntsi tɤɲi pjɯ-sɤtse ɯ-jaʁ ɯ-ntsi kɯ tshitsuku ɲɯ-znɤme qhe,
%    even like that (despites the pain in her legs) XXXX 14-tApitaRi, 52
...
The combinations $\dagger$\forme{kɯ kɯnɤ} or $\dagger$\forme{kɯnɤ kɯ} are unattested, and not accepted by native speakers. The contrast between absolutive and ergative noun phrases is therefore neutralized in additive or scalar focus with \forme{kɯnɤ}.

 

 \subsubsection{Additive focus marker \forme{ri}} \label{sec:ri.additive} 
...
The restrictive focus construction can be combined with a scalar focus in \forme{kɯnɤ} (see §  \ref{sec:kWnA}), as in (\ref{ex:ma.kWme.kWnA}). In this example, \forme{kɯnɤ} has scope over the participial relative clause \forme{mu ma stɯsti ma kɯ-me}, whose head, interestingly, is the noun \japhug{mu}{female} inside the postpositional phrase with the exceptive \japhug{ma}{apart from}.
