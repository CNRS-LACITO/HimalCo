...
Genitive postpositional phrases can occur as predicates with a copula as \japhug{ɯʑɤɣ}{\textsc{3sg}:\textsc{gen}} in (\ref{ex:WZAG.pjAmaR}).
...
In example (\ref{ex:ftsoR.kWngWt.WphW}), the head noun following the genitive \japhug{srɯnloʁ}{ring} has no possessive prefix (unlike the normal possessive construction described in § \ref{ex:prefix.expression.of.possession} and § \ref{sec:gen.possession}), and the genitive phrase describes a property associated with the head noun, rather than a possessor.
...
In (\ref{ex:slama.ra.GW}), the genitive phrase \forme{slama ra ɣɯ} is best translated as `concerning the students'.
