...
The  determiner \japhug{tɯka}{each} can also be used without a possessor in  \forme{raŋri}, for example with the distributive pronoun \japhug{ʑakastaka}{each his own}   as in (\ref{ex:nWkho.tWka}).
...
In addition to possessors, \japhug{tɯka}{each} also follows objects, with scope on the whole action (\ref{ex:pCaR.tWka}).
...
\end{exe}
 ...
 The indefinite \forme{ci} is not obligatory for indefinite referents (whether specific or non-specific), and bare NPs can used as \japhug{fsapaʁ}{animal} and \japhug{qapar}{dhole} in example (\ref{ex:ci.ra}).
