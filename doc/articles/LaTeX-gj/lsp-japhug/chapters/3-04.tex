...
While some verbs referring to natural forces are intransitive (\japhug{mnu}{share (as an earthquake}, § XXX), most are morphologically transitive auxiliary verbs (mainly \japhug{lɤt}{throw} and \japhug{βzu}{make}), used in collocation with a noun in absolutive form, as in (\ref{ex:tWmW.YAsWlAt} and (\ref{ex:qale.taBzu}).\footnote{The presence of the progressive \forme{asɯ-} prefix in (\ref{ex:tWmW.YAsWlAt}) and of the direct perfective \forme{ta-} prefix in (\ref{ex:qale.taBzu}) show that these verbs are morphologically transitive in these examples, see § XXX.}
...
In these collocations, the noun is however distinct from an object, as it is unexpectedly relativized with subject participial relatives in \forme{kɯ-}, see § XXX.
...
Semi-objects are relativized either with object participles in \forme{kɤ-} (§ XXX) or with finite relative clauses (§ XXX). Semi-objects are also found is some triactantial verbs. In particular, the theme of secundative verbs is a type of semi-object (§ \ref{sec:theme.ditransitive}).
...
The theme of secundative verb is however relativized like an object (see § XXX), and it can be considered as a sub-type of semi-object (§ \ref{sec:semi.object}).
...
\gll ɯ-ɣli tɤ-rɤku ɯ-ŋgɯ tɯ-ɣli wuma ʑo pe tɕe \\
\textsc{3sg}.\textsc{poss}-dung \textsc{indef}.\textsc{poss}-crops \textsc{3sg}.\textsc{poss}-inside \textsc{indef}.\textsc{poss}-dung really \textsc{emph} be.good:\textsc{fact} \textsc{lnk} \\
\glt `Its dung is a very good fertilizer (to put in the fields for) the crops.' (05-qaZo, 48)
\end{exe}
...
Goals can be relativized using oblique participial relatives (§ XXX) or finite relatives (§ XXX).
