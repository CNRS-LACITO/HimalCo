...
The secutive with third person singular possessive \forme{ɯ-rca} is also used as a linker meaning `in addition' (see § XXX). With the indefinite possessive prefix \forme{tɤ-rca} and  \forme{tɯ-tɯ-rca}, the secutive appears in adverbial function with the meaning `together' (§ XXX). In addition, the unexpected focus marker \forme{rcanɯ} (§ \ref{sec:unexpected}) and the dubitative sentence final particle \forme{rca} are historically related to this relator noun.
...
Another possibility to express restrictive focus is the use of the adverb \japhug{ʁɟa}{completely, all} with scope on a  noun phrase rather than the whole clause as in (\ref{ex:RJa.tunWndze}).
...
The word \forme{ʁɟa} also occurs as a postnominal modifier `bare' (§ \ref{ex:attributive.postnominal}), from which possibly both the uses as a restrictive marker, and as an adverb `completely', are independently derived.

%χtɕɤnzɤn ʁɟɯ-ʁɟa kɯ ʑo pɯ́-wɣ-nɤjo ɕti ɲɯ-ŋu. 
...
The word \forme{ʁɟa} which is most commonly used either as an adverb meaning `completely, all' (§ XXX) or as a restrictive focus marker `only' (§ \ref{sec:restrictive.focus}), also have marginal uses as a postnominal modifier meaning `bare', which is probably its etymological meaning,\footnote{The form \forme{ʁɟa} possibly originates from the first syllable of Tibetan \tibet{གཡའ་མ་}{gja.ma}{stone slab}, through a meaning `bare rock'.}  as in (\ref{ex:stAmku.RJa}) and (\ref{ex:Wru.RJa.nW}), though even in these examples the restrictive sense `only' is not absent. 
...
\ex \label{ex:Wru.RJa.nW}
 ...
 \textsc{3sg}.\textsc{poss}-grain \textsc{pfv}-not.exist \textsc{lnk} \textsc{3sg}.\textsc{poss}-stalk bare \textsc{dem} \textsc{pfv}-\textsc{nmlz}:P-thresh \textsc{dem} feeding.emmer \textsc{3sg}.\textsc{poss}-inside \textsc{ipfv}-\textsc{inv}-put.in \textsc{lnk} \\
...
Another trace of \forme{ʁɟa} as a noun is the denominal verb \japhug{aʁɟa}{be bald, be bare} (see § XXX on the \forme{a-} derivation), which can be applied to nouns such as \japhug{stɤmku}{grassland} and \japhug{zgo}{mountain}.
