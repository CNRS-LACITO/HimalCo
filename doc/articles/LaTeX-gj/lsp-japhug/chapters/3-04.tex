...
The dual can also be used with two nouns in apposition (§ \ref{sec:apposition}), as in (\ref{ex:Wmu.Wwa.ni}). 

\begin{exe}
\ex \label{ex:Wmu.Wwa.ni}
\gll   ɯ-mu ɯ-wa ni kɯ ɲɯ-z-nɤja-ndʑi qʰe \\
\textsc{3sg}.\textsc{poss}-mother \textsc{du} \textsc{erg} \textsc{ipfv}-\textsc{caus}-be.a.pity-\textsc{du} \textsc{lnk} \\
\glt `Her parents would not be parted from her.' (14-tApitaRi, 305)
\end{exe}

\subsubsection{Plural} \label{sec:plural.determiners}
The etymology of the plural marker \forme{ra} is unknown, but a potential cognate exists in Pumi (\forme{=ɹə}, (\citealt[135]{daudey14grammar}; Japhug \forme{-a} regularly corresponds to Pumi \forme{-ə} in the native vocabulary).
...
The appositive expression for `parents' has a honorific variant, originally used for noblemen in the traditional society. It comprises the terms \japhug{tɤ-pa}{father} and \japhug{tɤ-ma}{mother}, which are borrowed from Tibetan \tibet{ཨ་ཕ་}{ʔa.pʰa}{father}  and \tibet{ཨ་མ་}{ʔa.ma}{mother} respectively. Interesting, the honorific expression follows the `father-mother' order (as in example \ref{ex:apa.ama}), while the native expression puts `mother' in the first place.
