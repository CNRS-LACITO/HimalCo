...
Following locative adverbs (or locative postpositional phrases), \forme{nɯ} and its variants  have the meaning `the one/those X' as in (\ref{ex:aki.nW}). Note that the number determiners can also be used in the same way (example \ref{ex:alo.ra} § \ref{sec:plural.determiners})
...
\glt `Everybody in the family liked her very much.' (140429 qingwa wangzi, 5)
 ...
 A third universal quantifier, \japhug{lonba}{all} (from Tibetan \tibet{ལོན་པ་}{lon.pa}{reached, enough, completed}), exists in Japhug, but it is not used as a noun determiner and only occurs as an adverb (§ XXX).
...
The form \forme{tɕe}, which originates from a locative postposition (§\ref{sec:locative.j}), is mainly used in Japhug as a linker (§ XXX), one of the most common words in the corpus.
