...
Example (\ref{ex:nAmkha.zW.pjAnWlhoRnW}) shows the locative \forme{zɯ} expressing static location (ìn their hands') and motion from a place (`from the sky').
...
As other relator nouns, they can follow noun phrases (including headless relative clauses), with the possessive prefix agreeing in person and number with the preceding constituant, but can also occur on their own if the referent is definite, as \japhug{ɯ-taʁ}{on} in (\ref{ex:WtaR.zW.kAmdzW}) and (\ref{ex:WtaR.nWtCu}) below.
...
The relator noun \japhug{ɯ-stu}{straight ahead} is mainly used as an adverb, but can also serve to indicate the goal of a motion verb as in (\ref{ex:Wstu.Zo.YAGi}).
