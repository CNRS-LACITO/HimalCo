...
\glt `One's father's sister's children have to call oneself `my maternal uncle' if one is a boy, `my maternal aunt' if one is a girl.'  (see § XXX about Omaha kinship, 140425kWmdza03, 1)
\end{exe} 

As examples (\ref{ex:pjWkWZGAGANgi}) to (\ref{ex:tWZo.tWCki}) illustrate, generic agreement between pronoun, possessive prefix and verb indexation is very systematic. Examples of \textsc{1pl} indexation with generic pronouns or vice-versa are however attested (§\ref{sec:collective.indexation}).
...
\textsc{3sg}.\textsc{poss}-body what \textsc{nmlz}:S/A-be.like \textsc{dem} where \textsc{3sg-nmlz:obl}-remain \textsc{dem} \textsc{gen}  nmlz:S/A-fit \textsc{nmlz}:S/A-be.like \textsc{sens}-be:\textsc{affirm} \textsc{lnk} \\
...
\glt `What other (plants of the gender) \textit{allium} are there?' (07-Cku, 97)
\end{exe}   
...
\textsc{3sg}.\textsc{poss}-grandfather \textsc{nmlz}:S/A-have.a.child \textsc{pfv}-go[II] \textsc{sfp} \textsc{qu} \textsc{indef}.\textsc{poss}-son \textsc{ipfv}-\textsc{nmlz}:S/A-have.a.child when \textsc{ifr}.\textsc{ipfv}-be.usually.the.case \\
\glt `(You say) that you father-in-law went away to give birth to a child, since when can a man bear children?' (tAwa kWCqraR, 99)
\end{exe} 
...
\textsc{dem} where \textsc{2sg}.\textsc{poss}-hand \textsc{pfv}-come[II] be:\textsc{fact} where \textsc{pfv}-2-bring \\
...
more where dem emph be.poisonous:fact neg-know:genr lnk \\
...
Non-specific free-choice (\citealt[48]{haspelmath97indef}) indefinite referents can be expressed by interrogative pronouns in Japhug. One type of construction where this function is attested is correlatives, as in (\ref{ex:NotCu.lAtWrNgW}) and (\ref{ex:thAjtCu.fsaN}).
...
Examples of this construction are also found with the pronoun \japhug{ŋotɕu}{where}, with the meaning `no matter where, wherever' (location or direction from or to), as in (\ref{ex:NotCu.nWnWlhWlhoR}) and (\ref{ex:nWGtWta}). This free-choice indefinite meaning of  \forme{ŋotɕu} in also found in the delocutive expression \japhug{ŋɤtɕɯkɤti,kʰɯ}{obey to everything}, where the interrogative pronoun occurs in  \textit{status constructus} form \forme{ŋɤtɕɯ-} with the infinitive of \japhug{ti}{speak}  (§ \ref{sec:lexicalized.velar.inf}).
