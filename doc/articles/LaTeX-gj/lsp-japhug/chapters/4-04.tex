...
The negative prefix has the form \forme{mɯ-} when occurring with a perfective orientation prefix as \forme{mɯ-nɯ-kɯ-sna} `the one that is not good anymore' in (\ref{ex:mWnWkWsna}) and \forme{mɤ-} when no orientation prefix is present (examples \ref{ex:WmAkWmto} and \ref{ex:mAkWndza} above). 
...
\textsc{3sg}.\textsc{poss}-outside \textsc{lnk} \textsc{3sg}.\textsc{poss}-\textsc{transloc}-\textsc{nmlz}:P-work indef ifr.ipfv-exist lnk \\
...
There is rampant ambiguity between object participles, \forme{kɤ-} infinitives and subject participles of passive verbs. 
...
Due to the fact that passive verbs in Japhug are barely attested in perfective forms (§ XXX), participles with perfective prefixes can safely be considered to be unambiguous object participles, as in (\ref{ex:pWkAprAt}).
...
Infinitives in \forme{kɤ-} are superficially very similar to object participles, but unlike them, they can be formed with (non-semi-transitive) intransitive verbs (\ref{sec:velar.inf}). The main criterion to discriminate whether a \forme{kɤ-} form in a particular construction is an infinitive or an object participle is thus whether this particular construction is compatible with both transitive and intransitive verbs. 

\subsubsection{Object relative clauses}

\subsubsection{Complement clauses}

\subsubsection{Lexicalized object participles}
...
The second one, \japhug{ɯ-mɲoz}{preparation}, only occurs to express `provisions/preparation to avoid something getting damaged' as in (\ref{ex:WmYoz.tuwGBzu}), is derived from the transitive verb \japhug{mɲo}{prepare}, itself the irregular causative of \japhug{ɲo}{be prepared} (§ XXX).
...
\glt `One has to take preparation before it is damaged.' (elicited)
\end{exe}
