...
\subsection{Difference between AM and MCV}
...
\toprule
 & 	\ipa{-me gene-} & 	\ipa{-me ji-} & 	\ipa{-na/-ne} & 	\ipa{-nji} & 	\\
 \midrule
...
As pointed out by works on the typology of AM such as \citet[3]{guillaume16am}, it is common for AM to have additional meanings not involving a motion event distinct from that of the main verb. Manchu is such a language: the suffixes \ipa{-na/-ne} and \ipa{-nji} do not exclusively express AM meanings. 

Some motion verbs such as \ipa{isi-} `‘to reach, arrive, approach, come up' are neutral as to the cislocative / translocative contrast, and the suffixes \ipa{-na/-ne} and \ipa{-nji} are very commonly added to such verbs to specify motion towards or from the point of reference, without implying an additional motion event (see \ref{ex:isinafi}).
...
In addition, we find some rare examples where  \ipa{-na/-ne} and \ipa{-nji} are used in contexts where no motion event took place, and where the cislocative/ translocative orientation is more abstract. 
...
In Manchu, the grammaticalization of \ipa{-nji} and \ipa{-na/-ne} must be discussed separately.
...
All references works on Manchu (XXX) agree that \ipa{X-nji} results from the coalescence of the imperfective converb \ipa{X-me} with the verb \ipa{ji-} `give'. Apocope of the vowel \ipa{-e-} is attested in Manchu in XXX, and the assimilation of the *\ipa{m} to the following \ipa{j} is also found (CORRECT IF WRONG; ADD DATA IF APPROPRIATE)
...
\subsection{The translocative \ipa{-na/-ne} }
Unlike \ipa{-nji}, the Manchu translocative \ipa{-na/-ne} is a suffix of proto-Tungusic pedigree, attested in all other Tungusic languages, where it however has both cislocative and translocative functions, as shown by the following examples from Udihe:
...
There is no comparable MVC in this language with a verb in imperfective converbial form (CORRECT IF WRONG); rather, coordinated finite verb forms are used, as in (\ref{ngenejefi}).
...
Supposing a pathway such as \ipa{-na/-ne} $\leftarrow$ \ipa{-me gene-} would thus be anachronistic. Besides, since *\ipa{-ma ŋänä-} yields \ipa{-ŋaa} in languages other than Manchu, it is not possible to suppose that the AM suffix *\ipa{-nä-} was grammaticalized from *\ipa{-ma ŋänä-} in proto-Tungusic.

From the data at hand, two hypotheses are possible. First, proto-Tungusic *\ipa{-nä-} could be grammaticalized from the verb *\ipa{ŋänä-} `go', possibly through a coordinated construction rather than a MVC with purposive complement. (CORRECT IF WRONG)

Second, the verb *\ipa{ŋänä-} `go' could be a frozen translocative from a root *\ipa{ŋä-}. In this case, the origin of AM suffix *\ipa{-nä-} might not be recoverable by either the comparative method or internal reconstruction.
