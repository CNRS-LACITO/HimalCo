...
\ex \label{ex:efuleme}
\gll
 \ipa{boo} 	\ipa{be} 	\ipa{efule-me} 	\ipa{gene-fi} 	\ipa{.} 	\ipa{hoton-i} 	\ipa{hûlha} 	\ipa{de} 	\ipa{ilan} 	\ipa{tanggû} 	\ipa{fungeme} 	\ipa{meite-bu-he} \\
 house \textsc{acc} destroy-\textsc{ipfv.conv} go-\textsc{pfv.conv} { } city-\textsc{gen} rebel \textsc{dat} three hundred XX cut.off-\textsc{pass-pst} \\
...
Sibe went further than Classical Manchu in this regard. As pointed out by \citet[156]{zikmundova13sibe}, en MVC are partially grammaticalized in some contexts and have lost motional semantics, as show by example \ref{sex:sibe}, where the verb \ipa{ji-} `come' has become an aspectual marker.
...
Since AM is mainly attested in languages families whose history is poorly attested, the crosslinguistic study of the origins of AM is still a domain in infancy.

\citet{heine-kuteva02}


\citet{konnerth15cisloc} argues that grammaticalization occurred the other way round in Karbi, namely that the second person pronoun became an AM marker. This hypothesis, which strongly contradicts the unidirectionality of the pathway in XXX, needs to be confirmed by further study on the comparative phonology and morphology of the Karbi languages and its neighbours.
