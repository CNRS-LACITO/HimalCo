...
Previous scholarship on these AM and MCV in Manchu (\citealt{hayata95yuku}, \citealt{kubo97come}) focuses on their motional component (physical orientation, speaker’s and listener’s relative position, etc.) and some even concede that they are not synonymous (see below), though no clear account of the differences between the two structures has ever been proposed.

%For example, Hayata claims that AM forms mean ‘to go/come in order to V’, while MVC mean ‘to go/come V-ing’ 
...
The following minimal pair with the causative verb \ipa{ili-bu-} `build' (`cause to stand') in cislocative AM form (\ref{ex:ilibunjiha}) vs MVC (\ref{ex:ilibume}) illustrates the same semantic contrast as the one observed in Japhug. In (\ref{ex:ilibunjiha}), the focus is on the completion on the lexical verb, while the motion event is secondary and not even taken into account in the translation as in the above examples, while in (\ref{ex:ilibume}) it is clear that only the motion event has taken place, and that the action described by the purposive complement has not even started.
...
\ipa{hûlha-nji-ha} vs \ipa{hûlha-me} 	\ipa{ji-he} 
...
Context does not allow us to assume that the Department Magistrate came down to do the reports:
...
%\citet[156]{zikmundova13sibe} (2013: 156) argues that in Sibe, at least in some contexts, motion verb constructions are partially grammaticalized and have lost motional semantic nuances.
%
%\begin{exe}
%\ex 
%\gll
