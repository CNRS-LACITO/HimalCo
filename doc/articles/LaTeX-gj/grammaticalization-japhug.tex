...
\newcommand{\ipa}[1]{\mbox{\phon #1}} %API tjs en italique
...
Japhug and other Gyalrong languages have a productive derivation whereby a comitative adverb can be derived from a noun by removing all possessive prefixes, hadding the prefix \ipa{kɤɣɯ-} and partially reduplication the last syllable of the noun stem, as in \ipa{tɤ-rte} `hat' $\Rightarrow$ \ipa{kɤɣɯ-rtɯ\tld{}rte} `together with (his) hat', as illustrated by (\ref{ex:kAGWrtWrte}).
...
\subsubsection{Causative}  \label{ex:causative}
...
\end{exe}
\subsubsection{Passive} \label{ex:passive}
...
\subsubsection{Orientation prefixes}
...
While no direct pathway of grammaticalization \textsc{cislocative} $\rightarrow$ \textsc{egophoric} has ever been reported, there are clear cases of cislocatives becoming 2/3$\rightarrow$1 portmanteau person markers, in particular in direct-inverse systems (see \citealt{jacques14inverse}). While the exact pivot context which could allow reanalysis from centripetal/cislocative to egophoric is still unclear, it is valuable to explore in more detail this hypothesis.
...
The Egophoric and the Sensory are thus in near-complementary distribution, and in the few cases where both are possible with a verb in the same person form, the opposition in nearly always binary. 

Thus, the grammaticalization of a centrifugal marker as a sensory evidential in Gyalrong languages is not necessarily due to a semantic proximity between the two concepts. Rather, following the grammaticalization of the centripetal prefix as an egophoric, it is could result from the extension of the binary contrast between centripetal/cislocative vs centrifugal/translocative, to this newly acquired evidential value. 

\subsubsection{Progressive}
The progressive \ipa{asɯ-} / \ipa{ɤsɯ-} / \ipa{az-} / \ipa{ɤz-} differs from most TAM markers in being disyllabic (at least some of its allomorphs) and by the fact that two verb prefixes (the inverse and the autobenefactive, see example \ref{ex:pjAkAwGznAjoci} and \citealt{jacques15spontaneous}) can be infixed within it.
...
It can only be used with transitive verbs, and removes all markers of morphological transitivity (stem three alternation, past 1/3$\rightarrow$3 \ipa{-t} suffix) on the verb forms, as in (\ref{ex:asWndo}), where in the factual stem 3 \ipa{ndɤm} instead of stem 1 \ipa{ndo} would be expected.
...
\citet{jacques16prog} proposes to account for these two features by assuming that \ipa{asɯ-} originates from the combination of the agentless passive \ipa{a-} (on which see section \ref{ex:passive}) with the causative \ipa{sɯ-} (section \ref{ex:causative}). Since the passive in Japhug essentially has a stative overtone, 
XXXX

\section{Complex constructions} 
The present section focuses on a selected series of complex constructions involving linking elements for which a straightforward etymology can be proposed.  

  not including constructions borrowed from Tibetan (such as conditional in \ipa{nɤ} etc)
...
There are four different  adversative constructions in Japhug whose meaning can be translated as English  `not only ... but also'.
