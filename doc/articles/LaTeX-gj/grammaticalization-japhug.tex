...
\end{exe}
Here, the form of the topic remains \ipa{pɯpɯŋunɤ}, without agreement: if this still were a conjugated verb, the second person singular form shown in (\ref{ex:pWpWtWNunA1}) would have been expected instead.
...
or \ipa{scaʁa} `magpie' $\rightarrow$  \ipa{ascaʁa} `be white and black (like a magpie)'.\footnote{The last two examples are nouns borrowed from Tibetan.} 
...
&	&	stem&	prefixes\\
\midrule
Factual Non-Past&	\textsc{fact} &	1 or 3&	no prefix\\
...
\citet{jacques16prog} proposes to account for these two features by assuming that \ipa{asɯ-} originates from the combination of the agentless passive \ipa{a-} (on which see section \ref{ex:passive}) with the causative \ipa{sɯ-} (section \ref{ex:causative}).
XXXX
\section{Complex constructions} 
The present section focuses on a selected series of complex constructions (involving either subordinate clauses or correlatives) involving linking elements for which a straightforward etymology can be proposed.
