...
Stative verbs are a subclass of intransitive verbs, and can only index one argument, the intransitive subject (S), following the paradigm in Table \ref{tab:indexation} (the symbol \ro{} represents the verb stem, following the kirantologist tradition initiated by \citealt{driem93dumi}). Third person singular is zero-marked. The possessive prefixes, found on nouns and on some non-finite verb forms, are also indicated for comparison.
...
three-year \textsc{pfv}-pass-\textsc{1sg} so.much \textsc{emph} be.happy:\textsc{fact-1pl} be.in.good.terms:\textsc{fact-1pl} \\
