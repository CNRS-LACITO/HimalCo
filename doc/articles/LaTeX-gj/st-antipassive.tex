...
In Japhug for instance, the intransitive incorporating verb \ipa{ɣɯ-sɯ-pʰɯt} `chop firewood' derives from the transitive verb \ipa{pʰɯt} `cut, chop' and the noun  \ipa{si} `wood, tree' (incorporated in \textit{status constructus} form \ipa{sɯ-} with the denominal prefix \ipa{ɣɯ-}, see  \citealt{jacques12incorp}). Example (\ref{ex:kotshinW}) shows the transitive construction, with the subject taking the ergative \ipa{kɯ}, while (\ref{ex:koGWchAtshinW}) shows the corresponding incorporating construction, without ergative marking on the subject.

\begin{exe}
\ex \label{ex:kotshinW}
\gll \ipa{tɤ-tɕɯ} \ipa{ra} \ipa{kɯ} \ipa{cʰa} \ipa{ko-tsʰi-nɯ} \\
\textsc{3sg.poss}-son \textsc{pl}  \textsc{erg} alcohol \textsc{ifr}-drink-\textsc{pl} \\
\glt `The guys drank (the) alcohol.' (Japhug)
\end{exe}	 

\begin{exe}
\ex   \label{ex:koGWchAtshinW}
\gll \ipa{tɤ-tɕɯ} \ipa{ra} \ipa{ko-ɣɯ-cʰɤ-tsʰi-nɯ} \\
\textsc{3sg.poss}-son \textsc{pl}  \textsc{ifr-denom}-alcohol-drink \\
\glt `The guys drank alcohol.' (Japhug)
\end{exe}	 
