...
%Other non-AM meanings are however found.  One of interest is the voice reading found in Hayu, where with transitive verbs, \ipa{-la(t)}, which is otherwise an AM marker, is described as expressing the passive voice as in (\ref{ex:jeŋ.la}).
...
In the case of Tangsa, although \citet[311-312]{boro17hakhun} uses the term AM to refer to the construction in (\ref{ex:hakhun}), it remains unclear from the description whether an alternative analysis as MVC is possible.
...
Outside of Gyalrong and Kiranti, affixal AM probably exist in other branches, in particular Kuki-Chin (\citealt{chelliah17spatial}) and Dhimal (\citeyear[173-188]{king09dhimal}). In Dhimal, \citet{king09dhimal} identifies five `deictic motion' markers and provides extremely detailed description of their uses; three of these markers (all except the `indeterminate motion' \ipa{-gil} suffix and the `relinquitive \ipa{-dhi}) are clearly AM markers. Table (\ref{tab:dhimal.am}) summarizes the data.
...
%\ipa{gilli} `chase'  & Indeterminate &	\ipa{-gil} `go and do' &	P &	\textsc{trans}&	S/A & \Y \\
\bottomrule
\end{tabular}
\end{table}		

%Burmese has four preverbal (\ipa{la} `come', \ipa{θwà}  `go', \ipa{laiʔ} `follow, go around and', \ipa{cauʔ} `walk, go around and') and two postverbal auxiliaries (\ipa{-la} `come' and \ipa{-θwà/ðwà} `go') that can be used to express a motion event (\citealt[185-190;203-5]{Jenny16grammar}) and are candidates to be analyzed as AM.
%
...
AM systems have been grammaticalized from motion verbs independently several times in the TH family, and present considerable typological diversity. Table \ref{tab:TH} summarizes the main findings of this paper regarding affixal AM systems in the family.

\begin{table}[H]
\caption{AM systems in Trans-Himalayan languages} \centering \label{tab:TH}
\resizebox{\columnwidth}{!}{
\begin{tabular}{llllllllll}
\toprule
Subgroup&Language  &Number &Temp. & Deixis & Vertical & Other & Event  &Arg.\\
&&&relation &&dimension&functions& integration & \\
\midrule
...
%Sinitic languages have borderline cases of AM systems, as AM markers in these languages are clitics attached on the verb phrase rather than verbal affixes. This is also the case of Karbi and Hakhun Tangsa, the descriptions of which both mention AM, even though it is of a non-affixal variety.
...
%AM systems have been grammaticalized from motion verbs independently several times in the TH family, and present considerable typological diversity. Each of the three subgroups discussed in detail in this work contributes to the typology of AM in a different way.
%
...
%
%The present work contributes to the typology of AM by presenting in more detail a range of tests and criteria that can be used to refine the study of the semantic contrast between AM and other constructions. Previous descriptive work on AM in TH, even in the most detailed grammars, rarely provides explicit examples making it possible to determine whether pluriactional interpretation of the motion event + action is possible. The sharp contrast between Japhug on the one hand, and Khaling with regards to this parameter shows that fieldworkers should devote specific attention to this providing examples which make clear the presence or absence of this phenomenon.
