...
In the case of Tangsa, although \citet[311-312]{boro17hakhun} uses the term AM to refer to the construction in (\ref{ex:hakhun}), it remains unclear however from the description whether an alternative analysis as MVC is possible.
...
Outside of Gyalrong and Kiranti, affixal AM probably exist in other branches, in particular Kuki-Chin (\citealt{chelliah17spatial}) and Dhimal (\citeyear[173-188]{king09dhimal}). In Dhimal, \citet{king09dhimal} identifies five `deitic motion markers and provides extremely detailed description of their uses; three of these markers (all excepted the `indeterminate motion' \ipa{-gil} suffix and the `relinquitive \ipa{-dhi}) are clearly AM markers. Table (\ref{tab:dhimal.am}) summarizes the data.
