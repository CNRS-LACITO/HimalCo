...
%acknow: Linda Konnerth, two anonymous reviewers
...
\begin{table}[H]
\caption{The causative \ipa{-w-} infix in Tangut }\label{tab:prefixep} 
...
%{Causative and denominal} \label{sec:denominal}
...
%
%
%%As shown in \citet{jacques14antipassive} and \citet{jacques15causative}, the development from denominal to voice marker occurs in two stages.
%%First, the verb is nominalized into an action nominal consisting of the verb stem with a possessive prefix, without any nominalization marking (for instance, the transitive verb \forme{ɕpʰɤt}{patch} is nominalized as \forme{-ɕpʰɤt}{a patch}, a possessed noun). The transitivity of the verb is neutralized in this process. 
%%
%%Second, the action nominal undergoes denominal derivation into a new verb, whose transitivity and argument structure is determined by the denominal prefix. For instance, the antipassive \forme{rɤ-ɕpʰɤt}{patch things} from  \forme{ɕpʰɤt}{patch} can be historically (and perhaps even still synchronically) analyzed as being derived from \forme{-ɕpʰɤt}{a patch} by addition of \ipa{rɤ-} intransitive dynamic denominal prefix (the same prefix which derives for instance \ipa{rɤ-ma} `work (intr. v.)' from the possessed noun \ipa{-ma} `work (n)').
%
