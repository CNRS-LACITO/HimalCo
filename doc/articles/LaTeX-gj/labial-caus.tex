...
\item  \tgf{0735} \ipa{dʑjwij¹}:  \tgf{4052}\tgf{0749} \ipa{dạ²phji¹} `cause to become cold'
\end{itemize}
...
First, there is a recurrent correspondence of *\ipa{w} in Rgyalrongic languages in some words with labial stops in languages outside of Rgyalrongic, as exemplified by the verb `come' (Japhug \ipa{ɣi} `come', Tshobdun \ipa{wi} `come', \citealt[237]{sun14generic}, Khroskyabs \ipa{vjî}, \citealt[276]{lai15person}), whose cognates in Bodo-Garo and Kuki-Chin have labial stops (Bodo \ipa{pay} `come', Proto-Kuki-Chin *\ipa{paay} `go' \citealt{vanbik09pkc}). While the exact conditioning of this lenition in Rgyalrong languages (and its complex distribution across Rgyalrongic languages, not all which have lenition in the same words), the correspondence of stop to labial approximant observed with the labial causative is by no means exceptional.
...
There is some evidence for a denominal \ipa{b-} in Tibetan (\citealt[250-1]{zhang09cizu}). Evidence from transitive verbs suffers from the same problem as the hypothesized causative \ipa{b-}: it is unclear whether the \ipa{b-} is a real denominal prefix or a generalized past tense prefix reanalyzed as part of the stem.
...
\midrule
(st)& \ipa{tɤ-di} `smell' & \ipa{ɣɤ-di} `smell (badly)' \\
  (st)  & \ipa{tɤ-jwaʁ} `leaf' & \ipa{ɣɤ-jwaʁ} `growing leaves' \\
\midrule
...
There strong evidence in Japhug and other Rgyalrong languages than several voice prefixes are parallel to, and derived from denominal prefixes (see Table \ref{tab:denom}, \citealt{jacques14antipassive}). However, in the case of the causative \ipa{ɣɤ-}, the evidence is almost non-existant.
...
Denominal \ipa{ɣɤ-} deriving transitive verbs applies to very few examples (none from loanwords) and its meaning is not  causative, unlike denominal \ipa{sɯ(ɣ)- / ɕɯ(ɣ)-}, which shows pairs such as \ipa{(tɯ)-ɕtʂi} `sweat (n) $\rightarrow$ \ipa{sɯ-ɕtʂi} `to cause to sweat'. The great majority of denominal verbs in \ipa{ɣɤ-} are stative verbs. 
