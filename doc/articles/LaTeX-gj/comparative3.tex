...
\textsc{indef.poss}-water \textsc{pfv-nmlz}:S/A-be.mad \textsc{3sg}-inside \textsc{transloc-ipfv-inv}-throw have.to:\textsc{fact} \textsc{sens}-say-\textsc{pl} \textsc{inf:stat}-be.like \textsc{lnk} \textsc{qu-sens}-be \textsc{qu} \\
...
\gll 
 ...
 Yunnan \textsc{dem} \textsc{loc} exit:\textsc{fact} not.be:\textsc{fact} \textsc{polar:qu} \textsc{qu} \\
...
\gll 
 ...
 \textsc{2sg} \textsc{qu-2-}know:\textsc{fact} \textsc{polar:qu} \textsc{qu} \\
...
\section{Typological and historical perspectives}
...
Therefore, the historical origin of the multiclausal   degree construction in \ipa{kɯ}   (section \ref{sec:degree})   can be hypothesized to derive  from  the causal use of \ipa{kɯ}   with abstract nouns. %While derivation from the nominalized manner linking to the clausal degree construction would appear to be an alternative possibility for the origin of the multiclausal degree construction, it is unlikely in that only infinitives in \ipa{kɤ--} or \ipa{kɯ--} are found in nominalized manner linking.  

 
%  \subsubsection{\textsc{infinitival manner} $\rightarrow$ \textsc{finite manner}}
%
%As mentioned in section \ref{sec:linking}, the infinitive and participle prefixes in Japhug are very similar. Dynamic and stative verbs have infinitives in  \ipa{kɤ--} and \ipa{kɯ--} respectively. These prefixes are homophonous with the   the P-participle \ipa{kɤ--} and the S/A participle \ipa{kɯ--} prefixes respectively. It is easy to distinguish between infinitive and participle forms only in the case of intransitive dynamic verbs; intransitive stative verbs will have both their infinitive and S/A-participle marked with \ipa{kɯ--}.
%
%Thus, an infinitival clause can easily be reinterpreted as a headless relative in some cases; this ambiguity is synchronically attested in our Japhug corpus, where a surface form such as \ipa{kɯ-wɣrum} in \ref{ex:nmlz.inf.ambiguous} can be either analyzed as the stative infinitive of \ipa{wɣrum} `be white' or as its S/A participle `the white one'.
% 
% \begin{exe}
...
%
%From a sentence such as \ref{ex:nmlz.inf.ambiguous}, if the verbs of the manner clause are reanalyzed by the speakers as S/A participles, it can then be turned into a construction with finite verbs in both the main and the subordinate clauses.
%
...
%\textsc{2sg.poss}-place \textsc{neg-2}-stay:\textsc{fact} \textsc{erg?} water.beetle \textsc{emph} \textsc{sens}-2-be.like \\
