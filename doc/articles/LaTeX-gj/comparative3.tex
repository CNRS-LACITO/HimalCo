...
Therefore, the historical origin of the multiclausal   degree construction in \ipa{kɯ}   (section \ref{sec:degree})   can be hypothesized to derive  from  the causal use of \ipa{kɯ} (section \ref{sec:instr2cause}). While derivation from the nominalized manner linking to the clausal degree construction would appear to be an alternative possibility for the origin of the multiclause degree construction, it is unlikely in that only infinitives in \ipa{kɤ--} or \ipa{kɯ--} are found in nominalized manner linking.  
...
\textsc{lnk}   \textsc{inf:stat}-be.black \textsc{lnk} \textsc{inf:stat}-be.white \textsc{erg?} \textsc{nmlz}:S/A-be.piebald  \textsc{n.pst}:exist \\
...
\gll   \ipa{ɲɯ-ɲaʁ}  	\ipa{qhe}  	\ipa{ɲɯ-wɣrum}  	\ipa{kɯ} 	\ipa{ɲɯ-ɤscaʁa}  \\
   \textsc{const}-be.black \textsc{lnk} \textsc{const}-be.white \textsc{erg?} \textsc{const}-be.piebald  \ \\
\glt It is black and white so that ist is piebald. (Adapted from \ref{ex:nmlz.inf.ambiguous}) %revérifier avec cz
\end{exe}
...
\textsc{2sg.poss}-place \textsc{neg-2-n.pst}-stay \textsc{erg?} water.beetle \textsc{emph} \textsc{const}-2-be.like \\
...
\gll  
    ...
    again boulder \textsc{indef} exist:\textsc{sens} but \textsc{3sg.poss}-companion \textsc{pl} \textsc{inf:stat}-be.like \textsc{ipfv}-stay \textsc{neg:const}-can \textsc{ipfv}-rock.around \textsc{lnk} \textsc{ipfv}-rock.around  \textsc{const}-have.to \textsc{lnk}     \\
   \glt Then, there is this boulder, it cannot stay in place like other boulders as it rocks around continuously. (Divination  56)
 \end{exe}
 
 While a direct evolution from causative to adversative has been 
...
\citet{mauri12adversative} 
  ...
  First, it could be hypothesized that \ipa{kɯ} here derives from the topicalizer \ipa{kɤ-ti kɯ}  as the in the monoclausal degree construction (section \ref{sec:manner2adj}). This hypothesis is very unlikely however in view of the fact that, unlike in the degree construction, the marker \ipa{kɯ} in the comparative construction cannot be replaced by \ipa{kɤ-ti kɯ} and there is no evidence that it was ever  possible in any Rgyalrong language.
