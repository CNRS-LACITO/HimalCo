...
\title{From ergative to index of comparison} 
...
In Japhug Rgyalrong, clitic markers of the form \ipa{kɯ} appear in a wide array of different constructions. The most common one is the ergative marker, but we also find homophonous markers in causal linking, complementizer,  topicalizer, as the marker of the comparee in the comparative construction  or occurring after the nominalized stative verb  the degree construction.  

 While syncretisms between agent or ergative markers  on the one hand and various spatial cases on the other hand are common typologically (\citealt{agent02palancar}), it is unclear to what extent these various uses are related synchronically and diachronically, and whether one or several distinct markers have to be described.
...
\begin{exe} 
\ex \label{ex:instr3}
\gll   \ipa{qartshaz}   	\ipa{ɯ-ndʐi}   	\ipa{\textbf{kɯ}}   	\ipa{chɯ-βzu-nɯ}   	\ipa{tɕe,}   	\ipa{nɯ}   	\ipa{stu}   	\ipa{kɯ-ʑru.}     \\
 deer \textsc{3.sg.poss}-hide \textsc{erg} \textsc{ipfv}-do-\textsc{pl} \textsc{coord} \textsc{dem} most \textsc{nmlz:stat}-strong \\
...
Finally, the marker \ipa{kɯ} also occurs in the construction in \ref{ex:tWtWrpa.kW1} and \ref{ex:tWtWrpa.kW2} with a distributive after a classifier (`for one X', `per').

 \begin{exe} 
\ex \label{ex:tWtWrpa.kW1}
\gll  
\ipa{tɯ-tɯrpa} 	\ipa{\textbf{kɯ}} 	\ipa{sqi} 	\ipa{jamar} 	\ipa{ɲɯ-ra.} 	\\
one-pound \textsc{erg} ten about \textsc{const}-have.to \\
...
one-pound \textsc{erg} hundred \textsc{3sg.poss}-more \textsc{const}-come \\
...
\end{exe}  
 ...
 \subsection{Manner} \label{sec:manner}
The marker \ipa{kɯ}  appears after a infinitival subordinate clauses to indicate the circumstance or manner in which the action described by the verb of the main clause took place, as in \ref{ex:mAkApa.kW}. 
...
While \ipa{kɯ} cannot express cause on its own, it does appear in a clausal construction with the possessed noun `its strength' \ipa{ɯ-xɕɤt} borrowed from Tibetan \ipa{ɕed} `strength'.\footnote{This borrowed is specifically from Amdo, as it reflect an intermediate stage in the sound change from [ɕet] to [xet] as in most Amso varieties.}
...
\textsc{nmlz:action}-hurt \textsc{3sg.poss}-strength \textsc{erg} \textsc{1sg} \textsc{top} \textsc{1sg.poss}-head bone \textsc{evd}-have.a.crack \textsc{const}-be \textsc{pfv}-think-\textsc{pst-1sg} \\
...
\gll
  ...
  \textsc{indef.poss}-toil  \textsc{3sg.poss}-strength \textsc{erg} \textsc{ipfv}-die \textsc{n.pst}:be:\textsc{affirm} \\
 \glt (The bee) dies of exhaustion. (Bee, 40)
  \end{exe}
  
In this construction, we either find a nominalized verb form (in \ipa{tɯ--} or with the infitinitive \ipa{kɯ--} / \ipa{kɤ--}) as in \ref{ex:WxCAt.kW} or an abstract noun as in \ref{ex:WxCAt.kW2} in the causal subordinate clause.
...
\\
fir \textsc{inf:stat}-be.like \textsc{3sg.poss}-straight \textsc{ipfv:up}-go \textsc{const}-not.be \textsc{erg}? everywhere \textsc{3sg.poss}-branch const-come.out \textsc{const}-be \textsc{lnk} \\ 
...
\textsc{indef.poss}-water \textsc{pfv-nmlz}:S/A-be.mad \textsc{3sg}-inside \textsc{transloc-ipfv-inv}-throw \textsc{n.pst}:have.to \textsc{const}-say-\textsc{pl} \textsc{inf:stat}-be.like \textsc{lnk} \textsc{qu-const}-be \textsc{qu} \\
...
\gll 
 ...
 Yunnan \textsc{top} \textsc{loc} \textsc{n.pst:}exit \textsc{n.pst}:not.be \textsc{polar:qu} \textsc{qu} \\
...
yesterday \textsc{comparative} today \textsc{erg} \textsc{const}-warm \\
...
valley \textsc{gen} \textsc{dem} \textsc{erg?} \textsc{n.pst}:be.early because \textsc{dem} \textsc{loc} \textsc{erg?} \textsc{const}-be.warm \\
...
Pyracantha \textsc{gen} \textsc{3sg.poss}-leaf \textsc{erg?} \textsc{const}-be.big \\
...
\textsc{3sg.poss}-trunk a.little \textsc{n.pst}:be.like but plant.sp. \textsc{gen} \textsc{erg?} \textsc{n.pst}:be.high \\
...
but \textsc{3du.poss}-beak \textsc{pl} also, eagle \textsc{top} \textsc{erg?} \textsc{const}-be.black \\
...
\textsc{dem} \textsc{erg?} \textsc{3sg.poss}-price \textsc{const}-be.big \\
...
\gll 
  ...
  \textsc{3sg.poss}-ear \textsc{3sg.poss}-one.of.a.pair \textsc{top} \textsc{erg} \textsc{nmlz:S/A-redp}-be.small \textsc{const}-be.well but \textsc{dem} also much \textsc{neg:const}-be.good \\
...
Japhug has two constructions expressing the degree or further characterizing the property described by a adjective.\footnote{It is possible to define a category of adjectives in Japhug as a subclass of stative intransitive verbs: the verbs that allow the tropative derivation \ipa{nɤ--} (\citealt{jacques13tropative}).} A marker \ipa{kɯ} appears in both constructions.

\subsection{Nominalized degree construction} \label{sec:nmlz.degree}
Japhug has two degree constructions build by nominalizing a verb (generally an adjective) with the action nominalization \ipa{tɯ--} prefix and a possessive prefix coreferent with the referent presenting the property described by the nominalized verb. 

This degree nominal (like \ipa{ɯ-tɯ-tɕur} `its sourness' in \ref{ex:YWsWxtCur}) is the S of the construction.  There are two subtypes of nominalized degree construction, both illustrated by example \ref{ex:YWsWxtCur}. 

First, the degree nominal can be  combined with an adjective expressing a degree such as \ipa{saχaʁ} `be extremely ...', \ipa{sɤre} `be funny, be extremely ...', \ipa{tɕhom} `be excessive', in the \textit{adjectival nominalized degree} construction, exemplified by the first sentence in \ref{ex:YWsWxtCur}.

Second, it can be associated  with one or several  clause(s) containing a simile describing the degree of the property, in the \textit{clausal nominalized degree} construction. In clausal nominalized degree constructions, the nominalized verb has to be combined with the marker \ipa{kɯ}, as in the second sentence of example \ref{ex:YWsWxtCur}.
...
The nominalized verb and the marker \ipa{kɯ} of clausal  nominalized degree constructions form a constituent and can be right dislocated together, as in \ref{ex:WtWmbjom}.
...
\textsc{indef.poss}-water \textsc{3sg.poss}-side \textsc{top} \textsc{loc} \textsc{indef.poss}-wave \textsc{nmlz:S/A-redp}-be.small \textsc{top} a.little \textsc{ipfv}-go \textsc{const}-be apart.from \textsc{dem} apart.from   \textsc{nmlz:S/A}-be.clear not.exist:\textsc{sens} \textsc{3sg-nmlz:degree}-be.quick \textsc{erg?} \\
...
\gll 
 ...
 \textsc{lnk} \textsc{3du-nmlz:degree}-be.in.good.terms \textsc{3du-nmlz:degree}-be.happy \textsc{pst.ipfv}-be.extremely \textsc{emph} \textsc{const}-be \\
...
\glt I am starving.  (A calque from Chinese: \zh{我饿得不行了} Crow and Raven, 53) 
 ...
 The marker \ipa{kɯ} is not restricted to clausal nominalized degree construction, it also occurs optionally with the adjectival one, as in \ref{ex:kW.pWsaXaR}, though this use is very rare.
 
      \begin{exe}
  \ex  \label{ex:kW.pWsaXaR}  
  \gll 
   \ipa{tɤ-ɣɲat}  	\ipa{tɤ-mtsɯr}  	\ipa{kɯ}  	\ipa{pɯ-saχaʁ}  	\ipa{ʑo}  	\ipa{ɲɯ-ŋu}  \\
      \textsc{indef.poss}-be.tired     \textsc{indef.poss}-be.hungry \textsc{erg?} \textsc{pst.ipfv}-be.extremely \textsc{emph} \textsc{const}-be \\
...
commerce \textsc{inf}-make \textsc{3sg-nmlz:degree}-can \textsc{inf}-say \textsc{erg?} \textsc{pst.ipfv}-be.extremely \textsc{emph} \textsc{const}-be \\
...
%scuz>_tɤkhe pɣɤtɕɯ
\subsection{Non-nominalized degree construction} \label{sec:non.nmlz.degree}
A related construction comprising  a   negative finite clause   with the marker \ipa{kɯ} is also found. As in the degree construction, the  marker \ipa{kɯ} is followed by a  finite clause adding further information on the state or situation described by the previous clause, as in \ref{ex:mWjfse.kW}.
...
The optional marker \ipa{kɯ} on the comparee  clearly forms a constituent with the comparee NP (or its genitival modifier in a few limited cases), not with the verb. The comparee NP, though a S from the point of view of agreement with the predicate and from that of relativization, optionally receives  the same flagging as the A.

 
While many comparative constructions in the world's language do treat the comparee in the same way as the A (types B, C and E in \citealt[789]{dixon08comparative}'s survey, `exceed comparative' in  \citealt{stassen11comparative}), in all these constructions the standard has the same status as the O.  

The comparative construction in Japhug should be classifed differently. As the standard NP is marked by an oblique case (\ipa{sɤz} or \ipa{staʁ}) specific to this construction, and where the parameter of comparison is marked by an intransitive predicate, it belongs to  \citet{stassen11comparative}'s `particle comparative' type and to \citealt[789]{dixon08comparative}'s type A2.  While this type is not attested in combination with  ergative flagging in WALS (cf Table \ref{tab:stassen}, obtained by combining \citealt{stassen11comparative} with \citealt{comrie11case}), this may be due to the assignment of particular languages to the \textit{locative} rather than \textit{particle}  comparative types, and does not reflect a real gap in the data.
...
Syncretism between agent marker and distributive is well-attested in Romance languages. For instance, in French, the preposition  \ipa{par}  is used to mark the instrument, the (optional) agent in passive constructions and also occurs with a distributive meaning.
 
 
% Yet, the distributive meaning of \ipa{par} in French derives from the instrumental or from the spatial meanings is unclear. In the first case, it would offer a typological parallel to the development from instrumental to distributive of \ipa{kɯ} in  examples such as  \ref{ex:tWtWrpa.kW1} and  \ref{ex:tWtWrpa.kW2}. 

 \subsubsection{\textsc{instrumental} $\rightarrow$ *\textsc{cause}} \label{sec:instr2cause}
 In Japhug, the ergative \ipa{kɯ} can appear in causal clause linking only with the noun \ipa{ɯ-xɕɤt} `its strength' (from Tibetan \ipa{ɕed} `strength').
...
It is thus probable that the whole construction with \ipa{ɯ-xɕɤt kɯ} was borrowed from Tibetan together with the use of \ipa{kɯ} as a simple ergative / instrumental. It is possible to hypothesize that even though \ipa{kɯ} in Japhug cannot be used on its own in causal linkings, this function used to exist at an earlier stage as in Amdo; the hypothesis of a stage with \ipa{kɯ} as a causal marker will be necessary from the pathway in section \ref{sec:cause2degree}.
 
 \subsubsection{\textsc{cause} $\rightarrow$ \textsc{manner}}
...
\subsubsection{\textsc{cause} $\rightarrow$ \textsc{clausal nominalized degree} construction} \label{sec:cause2degree}

The origin of the  nominalized degree construction in \ipa{kɯ} with a complement clause (section \ref{sec:nmlz.degree})   can be hypothesized to derive  from  the causal use of \ipa{kɯ} (section \ref{sec:instr2cause}).

Such a development can be postulated due to the existence of potential  ambiguity between the causal and the degree interpretation. This can be illustrated with example \ref{ex:WtWrga.kW}.
...
In this construction, a causal interpretation would not be very different semantically from the degree interpretation; thus, the sentence \ref{ex:WtWrga.kW} would also make sense  with a causal interpretation (`She forgot it because of her being (so) happy').
...
Although the two meanings would appear to be entirely unrelated, the derivation from causal to degree is straightforward: for a property to be the cause of an event or a situation, this property must reach a sufficiently high degree to trigger a change of state or an action. Thus,  the causal construction necessary entails high degree, and evolution from the former to the latter is simply a restriction of the semantics of the construction.
  ...
  \subsubsection{\textsc{clausal nmlz degree} $\rightarrow$ \textsc{clausal non-nmlz degree}}

\begin{exe}
\ex
\gll \ipa{nɯnɯ}  	\ipa{kɯ-ɲaʁ}  	\ipa{ʁɟa}  	\ipa{tu,}  	\ipa{kɯ-wɣrum}  	\ipa{ʁɟa}  	\ipa{tu,}   \ipa{qhe}  	\ipa{kɯ-ɲaʁ}  	\ipa{qhe}  	\ipa{kɯ-wɣrum}  	\ipa{kɯ}  	\ipa{kɯ-ɤscaʁa}  	\ipa{tu,}  \\
\textsc{dem} \textsc{nmlz}:S/A-be.black completely \textsc{n.pst}:exist
\textsc{nmlz}:S/A-be.white completely \textsc{n.pst}:exist
\textsc{lnk}  \textsc{nmlz}:S/A-be.black \textsc{lnk} \textsc{nmlz}:S/A-be.white \textsc{erg?} \textsc{nmlz}:S/A-be.piebald \textsc{n.pst}:exist \\
...
\end{exe}
    ...
    mɤʑɯ rŋgɯ ci ɣɤʑu ri,
ɯ-zda ra kɯ-fse ku-rɤʑi mɯ́-j-khɯ kɯ tu-ɣɤzjɤɣlɤk nɤ tu-ɣɤzjɤɣlɤk ɲɯ-ra tɕe,
Divination  56
 
 "nɤ-stu mɤ-tɯ-rɤʑi kɯ tɯci paʁ ʑo ɲɯ-tɯ-fse" 
 tɯ-ci paʁ, 26
 
 \subsubsection{ \textsc{manner} $\rightarrow$  \textsc{adjectival nominalized degree}} \label{sec:manner2adj}
The  \ipa{kɯ} clitic marker, while obligatory in the Clausal nominalized degree construction, is very rare in the Adjectival one. Sentence \ref{ex:kW.pWsaXaR2}  (reproduced from \ref{ex:kW.pWsaXaR}) is the only such example in the whole corpus, but similar examples can be elicited.
...
One possible explanation to account for this is to suppose generalization from the Clausal nominalized construction. Yes, the fact that the adjectival nominalized degree construction commonly occurs with the infinitive of the verb `say' \ipa{kɤ-ti} in combination with   \ipa{kɯ} as in \ref{ex:kAti.kW2}  suggests otherwise.
      \begin{exe}
  \ex  \label{ex:kAti.kW2}  
  \gll 
\ipa{tɤ-rʑaβ} 	\ipa{ra} 	 	\ipa{nɯ-tɯ-rga} 	\ipa{kɤ-ti} 	\ipa{kɯ} 	\ipa{pɯ-saχaʁ} 	\ipa{ɲɯ-ŋu} 	\ipa{tɕe} \\
\textsc{indef.poss}-wife \textsc{pl} \textsc{3pl-nmlz:degree}-be.glad \textsc{inf}-say \textsc{erg?} \textsc{pst.impf}-be.extremely \textsc{const}-be \textsc{lnk} \\
...
Thus, the presence of the marker \ipa{kɯ} in Adjectival degree constructions is unrelated to that in Clausal degree constructions, and derives from the use of \ipa{kɯ} in manner subordinate clauses. 


 
 

 \subsubsection{\textsc{clausal non-nmlz degree} $\rightarrow$ \textsc{adversative} or \textsc{cause} $\rightarrow$ \textsc{adversative} }
  the emphatic adversative described in \ref{sec:advers}
 
\citet{mauri12adversative} 
 
 
 
  \subsubsection{\textsc{adversative} $\rightarrow$ \textsc{index} of comparison}
  As in most previous constructions, several hypotheses can be entertained to account for the origin of \ipa{kɯ} as a mark on the comparee as in example \ref{ex:kW.YWjpum}  (section \ref{sec:comparative}).
  
        \begin{exe}
  \ex  \label{ex:kW.YWjpum}  
  \gll 
  \ipa{tɯ-ɣli} 	\ipa{kɯ-dɤn} 	\ipa{ɯ-stu} 	\ipa{qandʐe} 	\ipa{nɯ} 	\ipa{kɯ} 	\ipa{ɲɯ-jpum.} \\
  \textsc{indef.poss}-excrement \textsc{nmlz}:S/A-be.many \textsc{3sg.poss}-place earthworm \textsc{top} \textsc{erg?} \textsc{const}-be.thick   \\
  \glt Earthworms (that are) in places rich in manure are thicker (than the other ones).
   (Earthworm, 125)
   \end{exe} 

  First, it could be hypothesized that \ipa{kɯ} here derives from the topicalizer \ipa{kɤ-ti kɯ}  as the in the adjectival degree construction (section \ref{sec:manner2adj}). This hypothesis is very unlikely however in view of the fact that, unlike in the degree construction, the marker \ipa{kɯ} in the comparative construction cannot be replaced by \ipa{kɤ-ti kɯ} and there is no evidence that it was ever  possible in any Rgyalrong language.
...
The first stage is attested in modern Japhug: an adversative construction with   adjectives in both  clauses, with the first adjective   negated and a comparative using the adverb \ipa{sthɯci} `as much' in the first clause as in \ref{ex:mAYaR.kW} and \ref{ex:mWYArtWm.kW}.  This construction is a variant of \citet{stassen11comparative}'s `conjoined comparative'.
  ...
  black.nettle \textsc{top} \textsc{3sg.poss}-colour \textsc{n.pst}:be.black a.little   white.nettle \textsc{top} \textsc{lnk} \textsc{dem} as.much neg-\textsc{n.pst}:be.black  \textsc{erg?} \textsc{n.pst}:be.yellowish \textsc{inf:stat}-be. like \\
...
\textsc{3sg.poss}-leaf \textsc{top} \textsc{pl} \textsc{1pl} \textsc{1pl.poss}-apple \textsc{neg-const}-be.round \textsc{erg?} \textsc{const}-be.long little \\
...
\textsc{3sg.poss}-leaf \textsc{top} \textsc{pl}  \textsc{neg-const}-be.round \textsc{erg?} \textsc{const}-be.long little \\
...
\textsc{3sg.poss}-leaf \textsc{top} \textsc{pl}   \textsc{erg?} \textsc{const}-be.long little \\
...
% its leaves are longer
   ...
   \begin{figure}
   \caption{Scenario of development from Ergative / Instrumental to the other functions of \ipa{kɯ}} \label{fig:scenario}  
  \begin{tikzpicture}
  \node (A) at (3,1) {\textbf{Ergative}/Instrumental};
   \node (B) at (-2,-1) {Distributive};
    \node (C) at (3.5,-1) {*Cause};
        \node (D) at (-2,-3)  {Clausal \textsc{nmlz} degree};
    \node (E) at (4,-3) {Manner}; 
        \node (F) at (-2.5,-5) {Clausal non-\textsc{nmlz} degree};
       \node (G) at (4.5,-5) {Quotative topic};
             \node (H) at (-3,-7) {Adversative};
             \node (I) at (5,-7) {Adjectival \textsc{nmlz} degree};
    \node (J) at (-3.5,-9) {\textbf{Comparee NP}};
...
\end{figure}
  ...
  \section{Conclusion}
  All functions of \ipa{kɯ} are related, except the sentence final particle
