...
Syncretisms between agent or ergative markers  on the one hand and various spatial cases on the other hand are common typologically (\citealt{agent02palancar}). However, the apparent polyfunctionality of the marker \ipa{kɯ} in Japhug, if indeed one marker has to be posited, is without equivalents and it is unclear to what extent these various uses are related synchronically and diachronically, and whether one or several distinct markers have to be described.
 ...
 In this paper, I provide a description of all the constructions where a clitic of the form \ipa{kɯ} is attested, from the ergative to the degree construction. Then, I propose a series of historical scenarios showing that most of  attested uses of this marker are diachronically related, and explaining how they  came into being.

Several previously unknown pathways of grammaticalization are proposed here for the first time on the basis of the Japhug data.
...
Additionally, as in many languages, the ergative is also used as an instrumental marker (\citealt[32]{agent02palancar}). When the instrumental in the ergative appears in the sentence, the verb is generally marked with the causative prefix \ipa{sɯ}-- / \ipa{z}--. It is possible  to have sentences with two noun phrases marked with the ergative (one A and one instrument) as in \ref{ex:instr3}, but such examples are extremely rare in our corpus.

 \begin{exe}
\ex \label{ex:instr}
\gll \ipa{kɯɕɯŋgɯ}   	\ipa{tɕe,}   	[\ipa{rɤɣo}]   	\ipa{\textbf{kɯ}}   	\ipa{tɯrju}   	\ipa{tu-sɯ-βzu-nɯ}   	\ipa{pjɤ-ŋu}   	\ipa{tɕe,}     \\
before \textsc{lnk} song \textsc{erg} word \textsc{ipfv}-\textsc{caus}-do-\textsc{pl} \textsc{ipfv:evd}-be \textsc{lnk}\\
...
\textsc{3pl.poss}-beak \textsc{dem} \textsc{erg} \textsc{ipfv-caus}-dig-\textsc{pl} \textsc{lnk} potentilla.anserina \textsc{ipfv-auto}-take.out-\textsc{pl} \textsc{ipfv}-be.usually.the.case \textsc{hearsay} \\
...
\gll
 ...
 lion \textsc{erg} \textsc{3sg.poss}-hand \textsc{erg} \textsc{top} \textsc{3sg.poss}-face \textsc{pl} \textsc{nmlz:action}-scratch \textsc{emph} \textsc{evd-caus}-throw \textsc{lnk}\\
...
\gll   [\ipa{qartsʰaz}   	\ipa{ɯ-ndʐi}]   	\ipa{\textbf{kɯ}}   	\ipa{cʰɯ-βzu-nɯ}   	\ipa{tɕe,}   	\ipa{nɯ}   	\ipa{stu}   	\ipa{kɯ-ʑru.}     \\
 deer \textsc{3.sg.poss}-hide \textsc{erg} \textsc{ipfv}-do-\textsc{pl} \textsc{lnk} \textsc{dem} most \textsc{nmlz:S/A}-strong \\
...
The agent is relativized by using the A participle, formed by prefixing the \ipa{kɯ--} nominalizing prefix  to the verb stem preceded by a possessive prefix coreferent with the P, as in \ref{ex:WkWnWmbrApW}.
\begin{exe}
...
On the other hand, the instrument is relativized by adding the oblique participle prefix \ipa{sɤ}-- and removing the causative prefix \ipa{sɯ}-- as in \ref{ex:sAxtCAr}.  
...
\glt `They only had threads to tie their waists (the only things that they could use to tie their waist were threads).' (Milaraspa translation)
 ...
 Outside of its use as agent or instrumental marker, \ipa{kɯ} is also selected by the transitive verb \ipa{kʰɤt} ``to do several times, a long time", where an abstract noun or an action nominalization with the ergative indicates the action realized.
 \begin{exe} 
\ex \label{ex:instr5}
...
one-pound \textsc{erg} hundred \textsc{3sg.poss}-more \textsc{3sg.poss}-price \textsc{testim}-come \\
...
\gll  
  ...
  one-month \textsc{lnk} 3sg.poss-salary hundred \textsc{3sg.poss}-more exist:\textsc{testim} \\
...
The marker \ipa{kɯ}  appears on   infinitival subordinate clauses  to indicate the circumstance or manner in which the action described by the verb of the main clause took place, as in \ref{ex:mAkApa.kW}. 
...
The marker \ipa{kɯ} can be used with an abstract noun to indicate cause, as in \ref{ex:sWmWzdWG.kW}, where \ipa{sɯmɯzdɯɣ}  `worry' is a noun borrowed from Tibetan \ipa{sems.sdug} (same meaning).
...
\textsc{nmlz:action}-hurt \textsc{3sg.poss}-strength \textsc{erg} \textsc{1sg} \textsc{dem} \textsc{1sg.poss}-head bone \textsc{evd}-have.a.crack \textsc{testim}-be \textsc{pfv}-think-\textsc{pst-1sg} \\
...
\gll
  ...
  \textsc{indef.poss}-toil  \textsc{3sg.poss}-strength \textsc{erg} \textsc{ipfv}-die be:\textsc{affirm}:\textsc{fact} \\
...
The comparee  bears a comparative marker when overt, either \ipa{sɤz} as in \ref{ex:comp1} and \ref{ex:comp3} or its variants \ipa{sɤznɤ}, \ipa{staʁ} and \ipa{staʁnɤ}. The comparative marker can only be dropped when a mark such as \ipa{stʰɯci} `that much, as much' is present (as in \ref{ex:sthWci.mA}).
...
However, in the case of Japhug, it is possible that the use of the marker \ipa{kɯ} in the distributive construction comes from its use with the verb \ipa{sɤndu} `exchange', as in \ref{ex:YWwGsAndu}.

  \begin{exe}
\ex \label{ex:YWwGsAndu}
\gll 
[\ipa{tɯ-tɯrpa}]  	\ipa{kɯ}  	\ipa{ɣurʑa}  	\ipa{jamar}  	\ipa{ɲɯ́-wɣ-sɤndu}  	\ipa{ɲɯ-khɯ}  \\
...
Sentence \ref{ex:YWwGsAndu} can be said by someone selling the product in question. Here the postpositional phrase  \ipa{tɯ-tɯrpa}  	\ipa{kɯ}  `with one pound' can be analyzed as an instrumental adjunct of the verb \ipa{sɤndu} `exchange'.  Such an analysis is not possible in sentences like \ref{ex:tWtWrpa.kW1.plus} (reproduced from \ref{ex:tWtWrpa.kW1}), representing the point of view of  the buyer.
...
The use of \ipa{kɯ} in the construction in \ref{ex:tWtWrpa.kW1.plus} can be accounted for by assuming reanalysis of the instrumental \ipa{kɯ} in \ref{ex:YWwGsAndu} as a distributive marker restricted to   expressing a price per unit or per quantity, and the subsequent generalization of this marker to constructions without the verb \ipa{sɤndu} `exchange'.


In the case of Japhug, unlike Romance, we do have evidence for the path \textsc{instrumental} $\rightarrow$ \textsc{distributive}.
...
Therefore, the historical origin of the multiclausal   degree construction in \ipa{kɯ}   (section \ref{sec:degree})   can be hypothesized to derive  from  the causal use of \ipa{kɯ}   with abstract nouns (attested in Japhug, see section \ref{sec:cause}). %While derivation from the nominalized manner linking to the clausal degree construction would appear to be an alternative possibility for the origin of the multiclausal degree construction, it is unlikely in that only infinitives in \ipa{kɤ--} or \ipa{kɯ--} are found in nominalized manner linking.  
...
From a sentence such as \ref{ex:nmlz.inf.ambiguous}, if the verbs of the manner clause are reanalyzed by the speakers as S/A nominalization, it can then be   turned into a construction with finite verbs in both the main and the subordinate clauses.
...
%\textsc{3sg.poss}-leaf \textsc{dem} \textsc{pl}  \textsc{neg-testim}-be.round \textsc{erg?} \textsc{testim}-be.long little \\
