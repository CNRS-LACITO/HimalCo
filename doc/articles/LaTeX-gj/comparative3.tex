...
Additionally, as in many languages, the ergative is also used as an instrumental marker (\citealt[32]{agent02palancar}). When the instrumental in the ergative appears in the sentence, the verb is generally marked with the causative prefix \ipa{sɯ}-- / \ipa{z}--. Although it is possible in elicited speech to have sentences with two noun phrases marked with the ergative (one A and one instrument), such examples are extremely rare in our corpus.
...
On the other hand, the instrument is relativized by adding the oblique participle in \ipa{sɤ}-- and removing the   causative prefix \ipa{sɯ}-- as in \ref{ex:sAxtCAr}.  
...
\textsc{lnk} \textsc{3sg}-inside \textsc{testim} \textsc{loc} pig.fodder \textsc{evd}-be.stuck \textsc{lnk}
...
go.back:\textsc{fact} \textsc{inf}-think \textsc{erg} \textsc{3sg.poss}-horse \textsc{testim} needle \textsc{evd-caus}-eat-\textsc{pl}   \textsc{3sg.poss}-dog \textsc{testim} toilet \textsc{3sg.poss}-down \textsc{evd}-pen-\textsc{pl} \\
...
\textsc{indef.poss}-shoe \textsc{testim} shape \textsc{neg-ipfv-inf:non.hum-anticaus}:change \textsc{erg} \textsc{ipfv-caus}-be.fixed \\
...
\textsc{lnk} \textsc{3sg.poss}-leaf \textsc{dem} other juniper \textsc{testim} \textsc{pl} \textsc{neg:testim}-be.like \textsc{erg?} \textsc{testim}-be.wrinkled \textsc{emph} \textsc{lnk}  \textsc{testim}-be.clustered.together \textsc{emph} \\
...
worry(n.) \textsc{erg} night \textsc{3sg.poss}-sleep at.all \textsc{neg-pfv}-come[II] \textsc{testim}-be \\
...
\textsc{nmlz:action}-hurt \textsc{3sg.poss}-strength \textsc{erg} \textsc{1sg} \textsc{testim} \textsc{1sg.poss}-head bone \textsc{evd}-have.a.crack \textsc{testim}-be \textsc{pfv}-think-\textsc{pst-1sg} \\
...
\gll 
 ...
 Yunnan \textsc{testim} \textsc{loc} \textsc{n.pst:}exit not.be:\textsc{fact} \textsc{polar:qu} \textsc{qu} \\
...
\gll 
    ...
    yesterday plant.sp. \textsc{testim} \textsc{pfv}-tell-\textsc{1sg} \textsc{polar:qu} \textsc{qu} \textsc{pfv}-think-\textsc{pst-1sg} \\
...
but \textsc{3du.poss}-beak \textsc{pl} also, eagle \textsc{testim} \textsc{erg?} \textsc{testim}-be.black \\
...
\textsc{3sg.poss}-tail \textsc{testim} mouse \textsc{gen} as.much \textsc{neg}-be.long:\textsc{fact} \\
...
beach willow \textsc{testim} \textsc{comp} as.much \textsc{neg:testim}-be.long \\
...
\gll 
  ...
  \textsc{3sg.poss}-ear \textsc{3sg.poss}-one.of.a.pair \textsc{testim} \textsc{erg} \textsc{nmlz:S/A-redp}-be.small \textsc{testim}-be.well but \textsc{dem} also much \textsc{neg:testim}-be.good \\
...
\textsc{coord} bucket.alcohol \textsc{dem} \textsc{erg} \textsc{inf:stat}-be.tasty \textsc{ipfv}-consider-\textsc{pl} be:\textsc{fact} \\
...
\textsc{indef.poss}-water \textsc{3sg.poss}-side \textsc{testim} \textsc{loc} \textsc{indef.poss}-wave \textsc{nmlz:S/A-redp}-be.small \textsc{testim} a.little \textsc{ipfv}-go \textsc{testim}-be apart.from \textsc{dem} apart.from   \textsc{nmlz:S/A}-be.clear not.exist:\textsc{sens} \textsc{3sg-nmlz:degree}-be.quick \textsc{erg?} \\
...
\textsc{indef.poss}-mother \textsc{testim} \textsc{erg} \textsc{3sg-nmlz:degree}-be.happy \textsc{erg?} \textsc{evd-auto}-forget  \textsc{lnk} \\
...
\gll  
 ...
 While a direct evolution from causative to adversative without intermediate stage as a manner linking has been documented in the history of Italian for instance (the linker \ipa{però}, see  \citealt{mauri12adversative}), the simplest explanation for the development of adversative \ipa{kɯ} in Japhug is a specialization of the use of the finite manner clause linking.
  ...
  \textsc{indef.poss}-excrement \textsc{nmlz}:S/A-be.many \textsc{3sg.poss}-place earthworm \textsc{testim} \textsc{erg?} \textsc{testim}-be.thick   \\
...
The first stage is attested in modern Japhug: an adversative construction with   adjectives in both  clauses, with the first adjective   negated and a comparative using the adverb \ipa{stʰɯci} `as much' in the first clause as in \ref{ex:mAYaR.kW} and \ref{ex:mWYArtWm.kW}.  This construction is a variant of \citet{stassen11comparative}'s `conjoined comparative'.
  ...
  black.nettle \textsc{testim} \textsc{3sg.poss}-colour be.black:\textsc{fact} a.little   white.nettle \textsc{testim} \textsc{lnk} \textsc{dem} as.much neg-be.black:\textsc{fact}  \textsc{erg?} be.yellowish:\textsc{fact} \textsc{inf:stat}-be. like \\
...
\textsc{3sg.poss}-leaf \textsc{testim} \textsc{pl} \textsc{1pl} \textsc{1pl.poss}-apple so.much \textsc{neg-testim}-be.round \textsc{erg?} \textsc{testim}-be.long little \\
...
%\textsc{3sg.poss}-leaf \textsc{testim} \textsc{pl}  \textsc{neg-testim}-be.round \textsc{erg?} \textsc{testim}-be.long little \\
...
\textsc{3sg.poss}-leaf \textsc{testim} \textsc{pl}   \textsc{erg?} \textsc{testim}-be.long little \\
