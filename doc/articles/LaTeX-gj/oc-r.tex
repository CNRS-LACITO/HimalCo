...
%The only clusters of this type in Tibetan are \ipa{dr-} and \ipa{ɴdr-}. The former has no unvoiced counterpart $\dagger$\ipa{tr} or $\dagger$\ipa{tʰr}  in the inherited vocabulary,\footnote{The cluster \ipa{tr-} only appears in a few loanwords and in placenames such as \ipa{Tre.hor}. } a gap which betrays the fact that the \ipa{d-} element is of prefixal origin. The prenasalized cluster \ipa{ɴdr-}  mainly comes from *\ipa{N-r} with epenthetic \ipa{d}(\citealt{lifk59globa, hill05vbri}).
