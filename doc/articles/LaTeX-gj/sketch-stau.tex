...
The verb \ipa{xi} is used with objects that are fixed in placed or plants that have grown on the ground, as in 

\ipa{akəstɞmbej} 	\ipa{zgɞrɟu} 	\ipa{qʰi} 	\ipa{nə-xi-sə} 	\ipa{ŋərə} 	\ipa{ŋə-rə} 

%\b	Le grand noyer de son voisin se trouvait près de la fenêtre d'Akustomba. 08

 
\ipa{ci/ɟi}

\ipa{tə/də}
%ɲi tə-pʰa stɔrmə le ndɔ ə́-də As-tu envie de te marier avec lui?

\ipa{kʰakʰa} 	\ipa{kʰarma} 	\ipa{rdʑi} 	\ipa{ɕatsa} 	\ipa{tə-rə} 	\ipa{ʑəgu} \ipa{ŋaɲɟi} 	\ipa{xi} 	\ipa{rdʑi} 	\ipa{de,} 	\ipa{rdʑi} 	\ipa{de,} 	\ipa{ŋaɲɟi} 	\ipa{aʑow} 	\ipa{taˠtaˠ} 	\ipa{χa} 	\ipa{kɔ-rə} 


%\b	Même s'il y avait beaucoup de traces de bétail,
%\b	les traces de notre mdzo, il les reconnaissait. 22-23
...
Finite complement clauses are  found with both phasal auxiliary verbs such as \ipa{ste}  `finish' (as in \ref{ex:xtse}; note that in this case, only the verb in the complement clause receives person indexation), and in reported speech.
\begin{exe}
\ex \label{ex:xtse}
\gll
