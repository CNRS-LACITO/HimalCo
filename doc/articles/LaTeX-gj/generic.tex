...
Also with copulas:XXXXX
tɕeri tɤpɤtso pɯ-kɯ-ŋu tɕe,
nɯ kɤ-ndza wuma ʑo pɯ-kɯ-rga.
Sambucus 135
...
\section{Nominalization in Japhug}
...
Nominalized forms cannot receive finite person marking (person is only marked by possessive prefixes), inverse \ipa{wɣ}--, direct \ipa{a}--, irrealis \ipa{a}-- or evidential directional prefixes.
...
The oblique nominalizer is only compatible with imperfective TAM prefixes, not with perfective ones. 
 ...
 \subsection{Use as converbs} 
 > NMLZ as converb in Japhug mɤ-kɯ-tsɯm mɤ-kɤ-pa

 \subsection{Nominalization and generic}  
Although the generic S/P \ipa{kɯ--} prefix and the S/A nominalization \ipa{kɯ--} prefix are homophonous and obviously historically related, they are nevertheless completely distinct synchronically in Japhug.

\section{Historical perspectives}

\subsection{Generic person marking and nominalization in Tshobdun}
...
\glt ‘If one eats poisonous mushrooms, one is made ill or is even killed if it is 
serious.’ 
 \end{exe}
 
 
\subsection{Nominalization in Situ}
The presently described varieties of Situ do  not have a generic marking system comparable to the ones in Japhug and Tshobdun. However, 
...
As shown in Table \ref{tab:genr:jpg:tshobdun}, although Japhug and Tshobdun are closely related languages and although the prefixes used in generic marking forms are cognate, the structure of the system is radically different. While Japhug presents clear ergative alignment in its generic marking pattern, with the inverse used to mark the generic A, Tshobdun exhibits accusative alignment as in nominalized forms.
...
The irregular generic A forms \ipa{mɤ-xsi} and \ipa{kɯ-ti}, remnants of
