...
\title{A note on volitional and non-volitional prefixes in Gyalrong languages\footnote{I would like to thank the reviewers, as well as Graham Thurgood and Ken-ichi Takashima for useful comments. The glosses follow the Leipzig Glossing Rules. Other abbreviations used here are:  \textsc{auto} autobenefactive / spontaneous,  \textsc{dem} demonstrative,  \textsc{emph} emphatic, \textsc{inv} inverse,  \textsc{lnk} linker, \textsc{pfv} perfective, \textsc{poss} possessor, \textsc{fact} factual,  \textsc{sens} sensory. Note that this uses bold instead of italics, because the contrast between \ipa{a} and \ipa{ɑ} is neutralized in italics. The Japhug examples are taken from a corpus that is progressively being made available on the Pangloss archive (\citealt{michailovsky14pangloss},  
...
The last example \ipa{pjoʔt} `fill up' $\rightarrow$ \ipa{məjot} `full'  in Table \ref{tab:nonvol} deserves a more detailed discussion. First, notice that some dialects of Situ  have the initial cluster \ipa{mɲ-} for the anticausative verb derived from   \ipa{pjōt} `fill' (Cogtse \ipa{mɲōt}  `full'), while others, like Kyomkyo, have instead a prefixal element \ipa{mə-} followed by \ipa{j-} (it is also the case in Bragdbar where we find \ipa{məjōt} `full', see \citealt[77]{zhang16bragdbar}). Dialects like Cogtse have clusters of the type \ipa{mC-} corresponding to either \ipa{məC-} or \ipa{mC-} in Kyomkyo and Bragdbar (see Table \ref{tab:mEC}).\footnote{Bragdbar data (from \citealt{zhang16bragdbar}) are used here due to greater accessibility, but agree with Kyomkyo (when the data is available) as regards to the presence or absence of a schwa. } It is therefore more likely to assume that Kyomkyo and Bradg-dbar are more archaic in preserving presyllables, while dialects like Cogtse have lost the vowel and created secondary clusters.
...
\gll \ipa{rtamtɕʰoʁ} 	\ipa{rɯnbotɕʰi} 	\ipa{kɯ-nɯci} 	\ipa{pɯ-ɣe} 	\ipa{ɲɯ-ŋu.} \ipa{tɕe} 	\ipa{nɯ} 	\ipa{χsɯ-ɣjɤn} 	\ipa{nɯ-nɤmbɣaʁlaʁ} 	\ipa{ɲɯ-ŋu.}  \\
...
\glt  (The horse) Rtamchog Rinpoche came down (from heaven) to drink water, and rolled three times (on the beach).' (Norbzang2012, 105)
 \end{exe}
...
We find however in Japhug an isolated example of a prefix \ipa{mɯ-} that might be related to volitionality, though not in the way proposed by Prins. Alongside the intransitive verb \ipa{mɯnmu} `move' which is attested in all Gyalrong languages (see Cogtse Situ \ipa{mənmō}, Kyomkyo Situ \ipa{mənmu}), we find another verb \ipa{nmu} `shake' which can only be used with earthquakes (see example \ref{ex:YAnmu} from \citealt{jacques16japhug}).\footnote{Note that the \ipa{ɣɤ-} prefix deriving a stative verb meaning `easy to X, tend to X' (\citealt[74]{jacques08}, \citealt{jackson14morpho}) can be applied to \ipa{nmu} `shake, occur (of earthquakes)', resulting in the verb \ipa{ɣɤ-nmu} `tend to occur (of earthquakes)'. }
