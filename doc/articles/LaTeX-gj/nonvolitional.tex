...
In all three examples, the base form is a noun and the derived one a verb. These examples are thus denominal derivations, not volitional markers. Note that the \ipa{mə-/ma-} denominal prefix in Situ\footnote{On the vocalism of the prefix, see the discussion on denominal in \citealt{jackson98morphology} and \citealt{linxr93jiarong} for instance.} does derive volitional verbs such as \ipa{tɑ-ʂpɑ̄k}  shoulder'  (\zh{肩膀}) $\rightarrow$ \ipa{mɑʂpɑ̄k} `carry on the shoulder' (\zh{扛}).
