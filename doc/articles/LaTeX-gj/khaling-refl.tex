...
\subsubsection{Passive, anticausative}

In the anticausative and passive interpretations of the \ipa{--si} derivation, the A of the base verb is demoted or deleted during derivation, and it is the P becomes the S of the derived verb. 
...
The distinction between anticausative and passive readings of the \ipa{-si} derivation is not always straightforward, but can generally be understood as follows: in the case of an anticausative reading, the A is irrecoverable both semantically and morphosyntactically, whereas in the case of a passive reading, the A is semantically present (in particular, as a generic human).

Speakers are quite firm in some cases that a single reading is possible. For instance, the verb |\ipa{pit-si}| (derived from |\ipa{pit}| `bring') in example \ref{ex:pansi} cannot have an anticausative reading: it cannot only be used to refer to people bringing water, not water coming by itself in pipes or hoses.

\begin{exe}
\ex \label{ex:pansi} 
\gll 
\ipa{ku}  	\ipa{mʌri}  	\ipa{pʌ̂n-si.}  \\
water much bring-\textsc{refl} \\
\glt A lot of water is brought over.
\end{exe}

Conversely, |\ipa{lent-si}| (derived from |\ipa{lent}| `to push down') in example \ref{ex:lentsi} refers to an action taking place spontaneously without an external actor, and thus does not allow a passive reading.

\begin{exe}
\ex \label{ex:lentsi} 
\gll 
\ipa{ʔʌ̄m}  	\ipa{kʌwʌ-bi}  	\ipa{mu-lēi-w-ɛ-si.}  \\
...
Because of the difficulty in recovering actors in some situations, a number of derived forms cannot formally be categorized as having an exclusive anticausative or passive reading.

For verbs expressing an action with beneficial results, a facilitative reading is found as well as a passive reading, as in example \ref{ex:kriptsi}. 

\begin{exe}
\ex \label{ex:kraptu} 
\gll 
\ipa{ʔuŋʌ}  	\ipa{sâː}  	\ipa{krʌpt-u.}  \\
\textsc{1sg:erg} vegetable cut-\textsc{1sg$\rightarrow$3} \\
\glt I cut the green vegetables.
\end{exe}
...
%\ex \label{ex:7yatsi} 
%\gll 
% \ipa{ʔɵdi}  	\ipa{mʉ-tōː}  	\ipa{ʔɛ̂n-si.}  \\
...
XXXX lop-si ``improve one's health'
