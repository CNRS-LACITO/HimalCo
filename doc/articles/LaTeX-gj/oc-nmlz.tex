...
\begin{exe}
\ex \label{ex:gzhob.tu.rlags}
\glt  \tibetan{བསྲེགས་ཤིང་གཞོབ་ཏུ་རླགས་ནས་དུ་བ་ཡང་མྱེད་ཐལ་བའི་ལྷག་མ་མྱེད་ཕ་}
\glt   \ipa{bsregs.ɕiŋ gʑob.tu rlags.nas du.ba jaŋ mʲed tʰal.baɦi lhag.ma mʲed.pʰa}
\glt  `(completely) burnt and destroyed in a fire, not even leaving smoke or ashes' 
\end{exe}
  In this passage, \tibetan{གཞོབ་} \ipa{gʑob} corresponds to \zh{火災}, with exactly the same meaning as the Japhug noun. The meaning `burning smell' might be due to an abbreviation of the compound  \tibet{གཞོབ་དྲི་}{gʑob.dri}{burning smell} in compounds. If this etymology is correct, the use of \tibetan{གཞོབ་} \ipa{gʑob} as a verb is due to zero conversion.
