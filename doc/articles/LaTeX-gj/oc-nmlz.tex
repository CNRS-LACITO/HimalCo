...
Gyalrongic languages have a set of prefixes deriving non-finite verb forms, including participles, converbs and infinitives (\citealt{jacques14linking}, \citealt{jackson14morpho}, \citealt{jacques16relatives}). 


\subsection{Velar participles}
Among these prefixes, we find the core argument participle prefixes (in Japhug \ipa{kɯ-} for subject and \ipa{kɤ-} for object participle), and related generic person and infinitive prefixes (\citealt{sun12complementation}, \citealt{sun14generic}, \citealt{jacques16complementation}, \citealt{jacques18generic}), which have cognate prefixes in many Trans-Himalayan languages, including Karbi and Kiranti (\citealt{konnerth16gV}) and have fossil reflexes in Tibetan (as \ipa{g-} or \ipa{d-} preinitials, see \citealt{jacques14snom}). There is evidence that the velar nominalization prefix in Tibetan was still marginally productive in Old Tibetan times, since the word \tibet{གཟུངས་}{gzuŋs}{dhāraṇī}, derived from the verb \tibet{འཛིན་}{ⁿdzin, bzuŋ}{seize, grasp} (root \dhat{zuŋ}) by calque from the Sanskrit \ipa{dhṛ} (the root from which \ipa{dhāraṇī} derives), has the nominalization circumfix \ipa{g-...-s}.\footnote{Circumfixal nominalization with a velar nominalization prefix is a feature also found in Limbu.}
...
The examples above are not problematic; note that in the case of honorific \tibet{སྙན་}{sɲan}{ear}, we possibly have a calque from Sanskrit \ipa{śrava-}, which is attested in the sense of `ear' in Classical Sanskrit. The adjective \tibet{སྙན་}{sɲan}{euphonious} XXX
