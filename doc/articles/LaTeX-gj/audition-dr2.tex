...
\newcommand{\bleu}[1]{{\color{blue}#1}}
 ...
 \title{Linguistique historique, terrain et formalisation}
 \author{Guillaume Jacques}
 \date{8 avril 2015}
 \maketitle
 \bibliographystyle{unified}
   \nobibliography{bibliogj}
 ...
 \begin{itemize}
\item  Recherches passées et en cours
\item  Programme de recherche
\item  Animation de la recherche et enseignement
\end{itemize}
   \end{frame} 
...
\end{itemize}
 ...
 \frametitle{Linguistique historique} 
  \framesubtitle{Phonologie et morphologie}  
...
\end{itemize}
 ...
 \frametitle{Projet de recherche} 
  ...
  \begin{tikzpicture}
    \node (B) at (0,0) {\rouge{Descriptions de langues}};
          \node (C) at (3,-2)  {\bleu{Linguistique historique}};
        \node (D) at (4,-4)  {\bleu{Linguistique sino-tibétaine}};
%       \node (F) at (3,-5)  {\bleu{Phonologie panchronique}};%\\ Adversative};
        \node (G) at (-3,-2) {\bleu{Typologie}}; 
             \node (H) at (0,-6)  {Modélisation de la méthode comparative};
\tikzstyle{peutetre}=[->,dotted,very thick,>=latex]
\tikzstyle{sur}=[->,very thick,>=latex]
\draw[sur] (B)--(C);
\draw[sur] (C)--(D);
...
\draw[peutetre] (C) to[bend left] (G);
\draw[peutetre] (G) to[bend left] (C);
\end{tikzpicture}
